		...
		if ((loaded_metrics == 0) && (typeof ym == 'function')) {
			if (meta_page.affiliate_tags != null) {
				...
				$.each(array, function( index, value ) {
					ym(32912255,'reachGoal',value);
					ym(32912255,'reachGoal','metka');
					console.log(value);
					loaded_metrics = 1;
				});
			}
						...
						if (verify_method == 'pay') {
							$.ajax({
								...
								success: function (data) {
									...
									window.location.href = '/dashboard/wallet/?price=1990&id='+result.poster_id;
								}
							});
						} else {
			...
			function(data) {
				...
				if(result.status === 'error') {
					...
					$('body').on('click', '.change_email_newform', function () {
						$('#after_login_new_comment_form input[type="submit"]').val('Далее');
					...
					var comment_id = $('#after_login_new_comment_form input[name="comment_id"]').val();
					var post_id = $('#after_login_new_comment_form input[name="post_id"]').val();
					$('#after_login_new_comment_form input[type="submit"]').val('Завершить регистрацию');
