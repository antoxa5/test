...
function ajax_comments(sort_type,userid) {
	...
	console.log(sort_type);
		$.ajax({
			...
			type: "POST",
			data: "action=ajax_comments&post_id="+current_post_id+"&sort_type="+sort_type+"&user_id="+userid+"&slug="+slug+"&type_c=single",
...
jQuery(document).ready(function($){
	...
	$('body').on('click', '.toc_list li a', function(){
		if ($(this).attr('href') == "#comments") {
			if ($(location).attr('hash') ==  '#comments') {
				if ($('#comments').size() == 0) {
					$('html, body').animate({
						scrollTop: $(".single_post_comments_top").offset().top-100
					}, 2000);
				}
			}
		}
