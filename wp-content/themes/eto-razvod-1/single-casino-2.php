...
if (have_posts()) :
	while (have_posts()) : the_post();
		...
		wp_enqueue_script( 'review_ajax_content', get_template_directory_uri() . '/js/review_ajax_content.js', array('jquery'),filemtime(TEMPLATEPATH . '/js/review_ajax_content.js') ); ?>

		<?php
		$cur_user_id = get_current_user_id();
		wp_enqueue_script( 'review_default', get_template_directory_uri() . '/js/review_default.js', array('jquery'),filemtime(TEMPLATEPATH . '/js/review_default.js') );
...
//} elseif(in_array($term_slug, array('learn2'))) {
		...
		<div class="page_content page_container review_container_content single_container">
			<div class="wrap">
				...
				if($current_language == 'en_US' || $current_language == 'fr_FR') {
				...
				if($content != '' || $term_slug == 'cryptocurrency') {
					...
					?>
					<div class="the_content the_content_2 color_dark_blue<?php echo $translation_no_class; ?>">
						...
						<div class="single_review_meta flex m_b_30">
							...
							<div class="single_review_date font_small color_dark_gray"><?php echo $update; ?><?php echo get_the_date('j F Y'); ?> <?php $current_language = get_locale(); if($current_language == 'ru_RU') { _e('года','er_theme'); }; ?>
								...
								<?php if (get_the_modified_date('j F Y') != get_the_date('j F Y')) { echo '<span class="updated updatedblock font_smaller_2">'.$update.''.get_the_modified_date('j F Y').' '; _e($city,'er_theme');echo '</span>'; }  ?></div>
		...
		<div class="page_content page_container background_light review_container_reviews single_container" <?php /*if(get_field('reviews_count_reviews',$post->ID) > 0) {*/ ?> itemscope itemtype="https://schema.org/LocalBusiness"<?php /*} */?> itemid="<?php echo get_the_permalink($post->ID);?>" >
			<?php
		...
		<div class="page_content page_container background_light review_container_abuses single_container">
			<div class="wrap">
				<div class="container_left">
					<ul id="abuses">
						<?php wp_enqueue_script( 'ajax-abuses', get_template_directory_uri() . '/js/ajax-abuses.js', array('jquery'),filemtime(TEMPLATEPATH . '/js/ajax-abuses.js')  ); ?>
					</ul>
					<div class="list_more_container"></div>
				</div>
				<div class="container_side flex flex_column">
