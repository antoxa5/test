...
foreach ($newarr as $key => $value) {
        ...
        if ((get_term($array_unique_var_temp_left_side[$key][0],'capabilities')->name) == 'Публикация статей в Блог') {
            $question = '<span class="f-question-tariff"><i class="fas fa-question"> <span class="showadvice"><span>2 статьи в месяц. Обязательная модерация со стороны редактора проекта при публикации статьи на сайт. Статья должна быть уникальной и соответвовать правилам сайта.</span></span></i></span>';
...
<?php if ((($block['id'] == 'block_5fc1057fc0df1')) || (($block['id'] == 'block_5fe4c26292fb7'))) { ?>
...
//console.log('1st');  
  $.ajax({
      ...
      type: "POST",
      data: "action=show_popup_serice_pager"+"&id="+id+"&pageid=<?php echo $numberpost; ?>",
...
.title_lefter .f-question-tariff i.fas.fa-question .showadvice {
    visibility: hidden;
    top: unset !important;
    z-index: 1;
    bottom: 27px;
...
td.item_first.name span.f-question-tariff {
    position: absolute;
    right: 5px;
}
