...
if(!function_exists('get_rating_fields_group')) {
	function get_rating_fields_group($post_id) {
		...
		if('casino' == get_post_type( $post_id )) {
			$term_slug = [];
			$term_slug_field = get_term( get_field('company_type',$post_id), 'companytypes' );
			if( isset( $term_slug_field->name ) ) {
				$term_slug = $term_slug_field->name;
			}
		...
		if($format == 'layout') {
			if ( $fields ) {
				...
				if(get_post_type($postID) == 'casino') {
					$print_total = floatval(get_field('reviews_rating_average',$postID));
				} else {
				...
				if($get_ratings_count['count'] > 0) {
					...
					$result .= '<span class="inner color_dark_blue font_bold font_20" itemprop="ratingValue">' . $ratingValue . '</span>';
					...
					$result .= '<span itemprop="itemReviewed" itemscope="" itemtype="https://schema.org/Organization">';
					$result .= '<meta itemprop="name" content="' . get_field( 'company_name', $postID ) . '" />';
					$result .= '</span>';
				...
				if($get_ratings_count['count'] > 0) {
					...
					if(get_query_var('comment_single_page')) {
						if($current_language == 'en_US') {
							...
							$result .= '<span class="number-stat-profile__number font_bolder text_centered m_b_20 color_dark_blue">' . count($comments_sorted) . ' ' . counted_text(count($comments_sorted), 'critique', 'critiques', 'critiques') . '</span>';
						} elseif($current_language == 'de_DE') {
							$result .= '<span class="number-stat-profile__number font_bolder text_centered m_b_20 color_dark_blue">' . count($comments_sorted) . ' ' . counted_text(count($comments_sorted), 'bewertung', 'bewertung', 'bewertungen') . '</span>';
						} elseif($current_language == 'es_ES') {
							$result .= '<span class="number-stat-profile__number font_bolder text_centered m_b_20 color_dark_blue">' . count($comments_sorted) . ' ' . counted_text(count($comments_sorted), 'opinión', 'opinión', 'opinión') . '</span>';
						} elseif($current_language == 'pl_PL') {
							$result .= '<span class="number-stat-profile__number font_bolder text_centered m_b_20 color_dark_blue">' . count($comments_sorted) . ' ' . counted_text(count($comments_sorted), 'opinia', 'opinia', 'opinia') . '</span>';
						} elseif($current_language == 'fi') {
							$result .= '<span class="number-stat-profile__number font_bolder text_centered m_b_20 color_dark_blue">' . count($comments_sorted) . ' ' . counted_text(count($comments_sorted), 'arvostelu', 'arvostelut', 'arvostelut') . '</span>';
						} elseif($current_language == 'id_ID') {
							$result .= '<span class="number-stat-profile__number font_bolder text_centered m_b_20 color_dark_blue">' . count($comments_sorted) . ' ' . counted_text(count($comments_sorted), 'ulasan', 'ulasan', 'ulasan') . '</span>';
						} else {
...
if(!function_exists('get_post_stars')) {
	function get_post_stars($group_id, $post_id_set = 0, $reviews_count = null) {
		global $post;
		$result = '';
		if($post_id_set != 0) {
			$post_id = $post_id_set;
...
if ($post_id_set != 0) {
	...
	if(get_post_type($post_id) == 'promocodes') {
		...
		$result .= '<div class="stars stars3 flex" data-id="'.$post_id_set.'" data-id-2="'.$postID.'"  data-id-3="'.$post_id.'">';
		$y = 0;
			...
			if( $percents > 0 ) {
				...
				if ( $y <= $percents ) {
					...
					if ( $y - floatval( $percents ) == 0.5 ) {
						...
						$class = 'half '.floatval( $percents ).' '.$y.' aaa'.($y - floatval( $percents ));
					}
				}
		...
		if (get_post_type($post_id) != 'promocodes') {
			if( $reviews_count > 0 ) {
				$result .= '<meta itemprop="bestRating" content="5">';
				$ratingValue = (floatval($percents) > 0.0) ? $percents : 1;
...
//hold
		...
		if (($data['slug'] == 'abuses') || ( $data['slug'] == 'dashboard_company' )) {
			...
			if($count_abuses > 0) {
				...
				$comments_top .= '<div class="abuses_sorter_title color_dark_gray pointer dropdown flex">' . __( 'Отсортировать', 'er_theme' ) . '</div>';
				$comments_top .= '<ul>';
				if ( $sort_type == 'new' ) {
					$sort_new_active = ' active';
				} else {
...
function popup_registration_final_social() {
	...
	$result .= '<div class="regform__line" id="login_temp_input" style="display: none">
                            <span class="text-login" name="login">
                                <span for="logintemp">https://etorazvod.ru/user/</span><input type="text" name="logintemp" placeholder="Создайте логин" id="logintemp"';
	if($social_username && $social_username != '') {
		$result .= 'value="'.$social_username.'"';
	};
...
function send_comment_abuse_form() {
	...
	if($_POST['abuse_text'] != '') {
		...
		$text .= __(' от пользователя ', 'er_theme');
		$text .= $user->display_name.' ('.$user->user_email.')';
		$text .= __('. Ссылка на комментарий: ', 'er_theme');
		$text .= get_comment_link($comment_id);
		$text .= __(' Текст жалобы: ','er_theme');
		$text .= $_POST['abuse_text'];
		$text .= '<img src="https://etorazvod.ru/engine/mail_update_status.php?key='.$mail_key.'" style="width:1px; height:1px;" />';
...
function comment_abuse_form() {
	...
	// $result .= '<span class="comment-date">'.get_comment_date('j F Y',$comment_id).' '.__('в','er_theme').' '.get_comment_date('H:i',$comment_id).'</span>';
	$result .= '<span class="comment-date">'.sprintf( __( '%s в %s', 'er_theme' ), get_comment_date( 'j F Y', $comment_id ), get_comment_date( 'H:i', $comment_id ) ).'</span>';
	$result .= '</div>';
	$result .= '</div>';
...
function comment_spam_form() {
	...
	if($mailResult == true) {
		...
		$current_language = get_locale();
		if ($current_language == 'en_US') {
			$message['message'] = __('Spam complaint sent','er_theme');
		} elseif ($current_language == 'de_DE') {
			$message['message'] = __('Spam-Beschwerde gesendet','er_theme');
		} elseif ($current_language == 'es_ES') {
			$message['message'] = __('Se ha enviado una queja por spam','er_theme');
		} elseif ($current_language == 'fr_FR') {
			$message['message'] = __('Envoi d`une plainte pour spam','er_theme');
		} else {
		...
		$current_language = get_locale();
		if ($current_language == 'en_US') {
			$message['message'] = __('Error in submitting a complaint. Please inform the administrator at info@revieweek.com','er_theme');
		} elseif ($current_language == 'de_DE') {
			$message['message'] = __('Fehler beim Absenden der Beschwerde. Bitte informieren Sie den Administrator unter  info@revieweek.de.','er_theme');
		} elseif ($current_language == 'es_ES') {
			$message['message'] = __('Error en el envío de la denuncia. Por favor, informe al administrador en info@revieweek.es.','er_theme');
		} elseif ($current_language == 'fr_FR') {
			$message['message'] = __("Erreur dans l'envoi de la plainte. Veuillez en informer l'administrateur à l'adresse info@revieweek.fr.",'er_theme');
		} else {
			$message['message'] = __('Ошибка отправки жалобы. Пожалуйста, сообщите администратору на info@eto-razvod.ru.','er_theme');
		}
	...
	$result .= '<div class="regform__line">
                        ...
                        <div class="regform__twocol">
                            ...
                            <div class="two-links-regform">
                                <a href="/forgot-password/" rel="nofollow" class="ihaveacc"'.__('Забыли пароль?','er_theme').'></a>
                            </div>
                        </div>
...
if($depth > 1) { ?>
    ...
    <li id="comment-<?php echo $comment->comment_ID; ?>" itemprop="review" itemscope itemtype="http://schema.org/Review" data-commentid="<?php echo $comment->comment_ID; ?>">
		<span itemprop="itemReviewed" itemscope="" itemtype="https://schema.org/Organization">
					<meta itemprop="name" content="<?php echo get_field('company_name', get_field('addpage_review', $comment->comment_post_ID) ); ?>" />
					<meta itemprop="telephone" content="<?php echo get_field('base_2_support_phones',get_field('addpage_review', $comment->comment_post_ID))[0]['text']; ?>" />
					<meta itemprop="address" content="<?php echo str_replace($cyr,$lat,get_field('company_main_office',get_field('addpage_review', $comment->comment_post_ID))); ?>" />
					<meta itemprop="url" content="<?php echo get_the_permalink( get_field('addpage_review', $comment->comment_post_ID) ); ?>" />
				</span>
	<?php
		...
		$text_a_user = '';
		if( $comment_author->first_name && !$comment_author->last_name ) { 
			$text_a_user = str_replace( $cyr, $lat, $comment_author->first_name );
		} elseif( !$comment_author->first_name && $comment_author->last_name ) { 
			$text_a_user = str_replace( $cyr, $lat, $comment_author->last_name );
		} elseif( $comment_author->first_name && $comment_author->last_name ) { 
			$text_a_user = str_replace( $cyr, $lat, $comment_author->first_name . ' ' . $comment_author->last_name ); 
		} else { 
			$text_a_user = str_replace( $cyr, $lat, $comment_author->user_nicename ); 
		}

		if( !strlen( $text_a_user ) ) {
			if ( $current_language == 'ru_RU' ) {
...
/*
        <span itemprop="itemReviewed" itemscope itemtype="https://schema.org/LocalBusiness">
			...
			<meta itemprop="address" content="<?php echo get_field('company_main_office',$comment->comment_post_ID); ?>" />
		</span>
*/
		?>
		<?php $comment_author = get_userdata( $comment->user_id ); ?>
		<span class="comment-author" itemprop="author" itemscope="" itemtype="https://schema.org/Person">
			<meta itemprop="name" content="<?php echo $text_a_user; ?>" />
			<?php echo $text_a_user; ?>
			<?php
			$comment_new = 1;
			if ($comment_new == 1) { ?>
				<?php
        ...
        <span class="comment-date" itemprop="datePublished" content="<?php comment_date( 'Y-m-d',$comment->comment_ID ); ?>T<?php comment_date( 'H:i:s',$comment->comment_ID ); ?>+00:00"><?php comment_date( 'j F Y',$comment->comment_ID ); ?> <?php echo __('в','er_theme'); ?> <?php comment_date( 'H:i',$comment->comment_ID ); ?></span><div itemprop="reviewBody" data-id="8314" class="comment_text comment_text_<?php echo $comment->comment_ID; ?> do_not_translate_css_class"><?php if (($comment->user_id == 31) || ($comment->user_id == 17) || ($comment->user_id == 15128)) {
				$current_language = get_locale();
				$textfrompost = $comment_data['content'];
				/*if($current_language != 'ru_RU') {
                        $translations = get_field('comment_translations','comment_'.$comment->comment_ID);
			...
			$textfull = trim(preg_replace('/ ezOneTab81939831 /', "\n", $textfull));$textfull = trim(preg_replace('/ ezTwoTab81939831 /', "\n\n", $textfull));$textfull = str_replace('ezTwoTab81939831', "", $textfull);$textfull = str_replace('amp;','',$textfull);echo apply_filters('the_content', $textfull); ?></div><a rel="nofollow" data-user-id="<?php echo $comment->user_id; ?>" class="comment-reply-link color_dark_blue pointer inactive"   data-commentid_v="61" data-commentid="<?php echo $comment->comment_ID; ?>" data-postid="<?php echo $comment->comment_post_ID; ?>" data-appendto="<?php comment_ID(); ?>" href="#"><?php _e('Ответить на отзыв','er_theme'); ?></a><?php
		
		$total_rating = floatval(get_comment_rating_values($args['rating_fields'],$comment,'total'));
		$total_rating_value = 5 / 100 * $total_rating;
		if ($total_rating != 0) {
			
			?><div  class="comment_total_rating" itemprop="reviewRating" itemscope="" itemtype="https://schema.org/Rating"><meta itemprop="bestRating" content = "5" /><meta itemprop="worstRating" content = "1" /><span itemprop="ratingValue"><?php echo (floatval(round($total_rating_value,1)) > 0.0) ? round($total_rating_value,1) : 1; ?></span></div><?php };
		?>
        ...
        <div class="comment-body body_<?php echo $comment->comment_ID; ?>" data-body-id="body_<?php echo $comment->comment_ID; ?>">
            <div class="comment_header flex">
                ...
                <div class="comment_meta">
                    ...
                    <div class="date-line"><span class="comment_to_id color_dark_gray font_small"></span> <span class="comment-date font_smaller color_dark_gray date_abuse" itemprop="datePublished" content="<?php comment_date( 'Y-m-d',$comment->comment_ID ); ?>T<?php comment_date( 'H:i:s',$comment->comment_ID ); ?>+00:00" data-profile="<?php comment_date('j F Y',$comment->comment_ID) ?>"><?php comment_date( 'j F Y',$comment->comment_ID ); ?> <?php echo __('в','er_theme'); ?> <?php comment_date( 'H:i',$comment->comment_ID ); ?></span></div>
                </div>
            ...
            <div class="comment_footer font_smaller">
				...
				if($comment_replies != 0) {
					?>
                    <div class="comment_reply_count dropdown flex color_dark_gray m_right_20 pointer comment_reply_count_get_abuse bbb"><?php _e('Развернуть','er_theme'); ?></div>
				<?php } ?>
...
<?php } else {
	...
	<li class="<?php echo $comment_class_print; ?> white_block" id="ajax-comment-<?php echo $comment->comment_ID; ?>" itemprop="review" itemscope itemtype="http://schema.org/Review"   data-commentid_v="4" data-commentid="<?php echo $comment->comment_ID; ?>" data-postid="<?php echo $post_ID_comment; ?>" data-company_t="6" data-company="<?php echo get_field('company_name',$post_ID_comment); ?>"  data-url="<?php echo get_field('company_redirect_key',$post_ID_comment); ?>" <?php echo $holded; ?>><?php echo $holded_span; ?>
		<meta itemprop="itemReviewed" content="<?php echo get_field('company_name',$post_ID_comment ); ?>" />
		<div class="comment-body body_<?php echo $comment->comment_ID; ?>" data-body-id="body_<?php echo $comment->comment_ID; ?>">
			<div class="comment_header flex">
				<?php $comment_author = get_userdata( $comment->user_id ); ?>
				<?php if (get_field('pub_profile', 'user_'.$comment->user_id) == 'yes') {
...
<?php if($par_aurhor->first_name && !$par_aurhor->last_name) { echo $par_aurhor->first_name; } elseif(!$par_aurhor->first_name && $par_aurhor->last_name) { echo $par_aurhor->last_name; } elseif($par_aurhor->first_name && $par_aurhor->last_name) { echo $par_aurhor->first_name.' '.$par_aurhor->last_name; } else { echo $par_aurhor->user_nicename; } ?>
					...
					<?php } else { ?>
						<div class="comment_author_wrapper m_b_5">
                            <span  class="comment-author font_bold font_small color_dark_blue pointer" itemprop="author" data-link="<?php echo get_bloginfo('url').'/user/'; ?>">
								...
								<?php _e($text_a_user,'er_theme'); ?>
                                                                                        </span>
						</div>
					<?php }; ?>
					<span class="comment-date font_smaller color_dark_gray" itemprop="datePublished" content="<?php comment_date( 'c', $comment->comment_ID ); ?>">
						<?php printf( __( '%s в %s', 'er_theme' ), get_comment_date( 'j F Y', $comment->comment_ID ), get_comment_date( 'H:i', $comment->comment_ID ) ); ?>
					</span>
					<?php
				...
				if ($total_rating != 0) {
					...
					?>
					<div data-value="<?=round($total_rating_value,1)?>" class="comment_total_rating" itemprop="reviewRating" itemscope="" itemtype="https://schema.org/Rating">
						<div class="review_average_round progress small pointer <?php echo $rating_color;?>" id="comment_rating_total_<?php echo $comment->comment_ID; ?>" data-percent="<?php echo $data_percent; ?>">
							<div class="inner color_dark_blue font_bold font_small pointer" itemprop="ratingValue"><?php echo (floatval(round($total_rating_value,1)) > 0.0) ? round($total_rating_value,1) : 1; ?></div>
			...
			<div class="comment-content">
				...
				<?php }; ?>
				<div itemprop="reviewBody" data-id="1913" class="comment_text comment_text_<?php echo $comment->comment_ID; ?> do_not_translate_css_class"><?php
					$current_language = get_locale();
					// if($comment_data['translated_from'] && $comment_data['translated_from'] != '') {
					// echo '<div class="translated_from">'.__('Переведено с','er_theme').' <span>'.$comment_data['translated_from'].'</span></div>';
					// }
					//$language_original = get_field('language_original','comment_'.$comment->comment_ID);
					/*if($current_language == 'en_US' && $language_original == 'ru_RU') {

							echo '<div class="translated_from">Translated from <span>RU</span></div>';
						} elseif($current_language == 'fr_FR' && $language_original == 'fr_FR') {
							echo '<div class="translated_from">Translated from <span>FR</span></div>';
						} elseif($current_language == 'fr_FR' && $language_original == 'ru_RU') {
							echo '<div class="translated_from">Traduit de <span>RU</span></div>';
						} elseif($current_language == 'fr_FR' && $language_original == 'en_US') {
							echo '<div class="translated_from">Traduit de <span>EN</span></div>';
						} elseif($current_language == 'ru_RU' && $language_original == 'fr_FR') {
							echo '<div class="translated_from">Переведено с <span>FR</span></div>';
					...
					$textfull = trim(preg_replace('/ ezOneTab81939831 /', "\n", $textfull));$textfull = trim(preg_replace('/ ezTwoTab81939831 /', "\n\n", $textfull));$textfull = str_replace('ezTwoTab81939831', "", $textfull);$textfull = str_replace('amp;','',$textfull);
					...
					if (get_post_type($comment->comment_post_ID) != 'page' && get_post_type($comment->comment_post_ID) != 'post') {
						$continue_reading = '<a href="'.get_the_permalink($comment->comment_post_ID).'comment-'.$comment->comment_ID.'/" class="continue_reading continue_3reading continue_3reading604">'.$continue_reading_btn.'</a>';
						$textfull = truncateToWord( $textfull, 300,$continue_reading );
					}

					//}

					echo apply_filters('the_content', $textfull); ?></div>
				<?php
				$comment_files ='';
				$comment_files = get_field('comment_files','comment_'.$comment->comment_ID);
				if(!empty($comment_files)) {
		...
		$review_minuses = get_field('review_minuses', 'comment_' . $comment->comment_ID);
				...
				if(!empty($review_pluses) || !empty($review_minuses)) {
					...
					if(!empty($review_pluses)) {
						...
						foreach ($review_pluses as $item) {
							if($item != '') {
								$result_pluses .= '<li class="do_not_translate_css_class">'.$item.'</li>';
							}
			...
			<div class="comment_footer comment_footer_parent font_smaller">
				...
				<?php } ?>
				<a rel="nofollow" data-user-id="<?php echo $comment->user_id; ?>" class="comment-reply-link color_dark_blue pointer inactive"   data-commentid_v="62" data-commentid="<?php echo $comment->comment_ID; ?>" data-postid="<?php echo $comment->comment_post_ID; ?>" data-appendto="<?php comment_ID(); ?>"><?php _e('Ответить на отзыв','er_theme'); ?></a>
				<div class="comment_footer_right hovered_<?php echo $comment->comment_ID; ?>">
				...
				if ($current_language == 'en_US') {
					$notrecommend = 'No Recommend';
				} elseif ($current_language == 'es_ES') {
					$notrecommend = 'No se recomienda';
				} elseif ($current_language == 'de_DE') {
					$notrecommend = 'Nicht empfohlen';
				} elseif ($current_language == 'fr_FR') {
					$notrecommend = 'Non recommandé';
				} else {
					$notrecommend = 'Не рекомендую';
				}
				?>
				<?php if(get_field('review_recommend_friends','comment_'.$comment->comment_ID) == 'yes') { ?>
					<div class="comment_recommend_yes"><?php _e($recommend,'er_theme'); ?></div>
				<?php } elseif(get_field('review_recommend_friends','comment_'.$comment->comment_ID) == 'no') { ?>
					<div class="comment_recommend_no"><?php _e($notrecommend,'er_theme'); ?></div>
				<?php } ?>
			</div>
		</div>
    ...
    <li class="<?php echo $comment_class_print; ?>" id="ajax-comment-<?php echo $comment->comment_ID; ?>" itemprop="review" itemscope itemtype="http://schema.org/Review"   data-commentid_v="11" data-commentid="<?php echo $comment->comment_ID; ?>">
        ...
        <div class="comment-body body_<?php echo $comment->comment_ID; ?>"  data-body-id="body_<?php echo $comment->comment_ID; ?>"  data-parentid="<?php echo $comment->comment_post_ID; ?>"  data-post-status="<?php echo get_post_status($comment->comment_post_ID); ?>" data-body-id="body_<?php echo $comment->comment_ID; ?>">
            <div class="comment_header">
                ...
                <div class="comment_meta">
                    ...
                    <span class="comment-date" itemprop="datePublished" content="<?php comment_date( 'c', $comment->comment_ID ); ?>">
						<?php printf( __( '%s в %s', 'er_theme' ), get_comment_date( 'j F Y', $comment->comment_ID ), get_comment_date( 'H:i', $comment->comment_ID ) ); ?>
					</span>
                </div>
				...
				if ($total_rating != 0) {
                    ...
                    <div  class="comment_total_rating" itemprop="reviewRating" itemscope="" itemtype="https://schema.org/Rating">
                        <div class="c100 p<?php echo round($total_rating,0); ?> small <?php echo $rating_color;?>">
                            <span itemprop="ratingValue"><?php echo (floatval(round($total_rating_value,1)) > 0.0) ? round($total_rating_value,1) : 1; ?></span>
                            <div class="slice">
                                <div class="bar1"></div>
                                <div class="fill"></div>
                            </div>
		...
		function get_comment_rating_values($rating,$comment,$format) {
			...
			if ($format == 'list') {
				if ( ! empty( $rating ) ) {
					...
					foreach ( $rating as $item ) {
						$row_index++;
						$rating_value = floatval(get_field( $item['field_name'], $comment )) * 10 * 2;
						if ( $rating_value >= 70 ) {
							$rating_color = 'green';
						} elseif ( $rating_value >= 40 && $rating_value < 70 ) {
							$rating_color = 'orange';
				...
				if ( ! empty( $rating ) ) {
					$total_rating = array();
					foreach ( $rating as $item ) {
						$rating_value = floatval(get_field( $item['field_name'], $comment ));
						$rating_value = $rating_value ? ( $rating_value * 10 * 2 ) : 0;
		...
		function add_file_by_link() {
			...
			if (filter_var($data['parse_link'], FILTER_VALIDATE_URL)) {
				...
				if(url_isImage($data['parse_link'])) {
					...
					if ( !is_wp_error( $temp_file ) ) {
						...
						if ( !empty( $results['error'] ) ) {
							...
							$current_language = get_locale();
							if ($current_language == 'en_US') {
								$result['message'] = __('Unable to attach this image','er_theme');
							} elseif ($current_language == 'de_DE') {
								$result['message'] = __('Dieses Bild kann nicht angehängt werden','er_theme');
							} elseif ($current_language == 'es_ES') {
								$result['message'] = __('No se puede adjuntar esta imagen','er_theme');
							} elseif ($current_language == 'fr_FR') {
								$result['message'] = __('Impossible de joindre cette image','er_theme');
							} else {
							...
							if (in_array($filetype, $arr_img_ext)) {
								...
								$current_language = get_locale();
								if ($current_language == 'en_US') {
									$result['message'] = __('The image has been successfully uploaded!','er_theme');
								} elseif ($current_language == 'de_DE') {
									$result['message'] = __('Das Bild wurde erfolgreich hochgeladen!','er_theme');
								} elseif ($current_language == 'es_ES') {
									$result['message'] = __('La imagen se ha cargado con éxito.','er_theme');
								} elseif ($current_language == 'fr_FR') {
									$result['message'] = __("L'image a été téléchargée avec succès !",'er_theme');
								} else {
				...
				$current_language = get_locale();
				if ($current_language == 'en_US') {
					$result['message'] = __('Please specify a valid URL','er_theme');
				} elseif ($current_language == 'de_DE') {
					$result['message'] = __('Dieses Bild kann nicht angehängt werden','er_theme');
				} elseif ($current_language == 'es_ES') {
					$result['message'] = __('No se puede adjuntar esta imagen','er_theme');
				} elseif ($current_language == 'fr_FR') {
					$result['message'] = __('Veuillez spécifier une URL valide','er_theme');
				} else {
...
//делаем ссылку
		...
		function get_comments_count($post_id,$ratings) {
			...
			if(!empty($ratings) && $ratings != 0) {
				...
				foreach($ratings as $item) {
					$item_count = 0;
					$item_sum = 0;
					if( isset( $result['ratings_total'][$item['field_name']]['values'] ) ) {
					...
					$result['ratings_total'][$item['field_name']]['total'] = ( !$item_count ) ? 0 : ( $item_sum / $item_count );
				}
		...
		function page_single_rating($post_id) {
			...
			foreach (range(1, 5) as $i) {
				...
				$res .= '<label for="page_single_rate_'.$i.'">';
				if($i == round($page_rate_rating)) {
					$res .= '<input type="radio" name="page_single_rate" id="page_single_rate_'.$i.'" value="'.$i.'" checked data-comment-id="'.$post_id.'" aria-label="'.$i.' star"/>';
				} else {
					$res .= '<input type="radio" name="page_single_rate" id="page_single_rate_'.$i.'" value="'.$i.'" data-comment-id="'.$post_id.'" aria-label="'.$i.' star"/>';
				}
			...
			if($page_rate_count && $page_rate_count != 0 && $page_rate_rating && $page_rate_rating != 0) {
				if (get_post_type($post_id) == 'addpages') {
					...
					$res .= '<span class="rating_page_text_rating">'.number_format( $page_rate_rating, 2, '.', '' ).'</span>';
					$res .= ' / ';
					$res .= '<span class="rating_page_text_count">'.$page_rate_count.'</span>';
					$res .= '</div>';
				} else {
					$res .= '<div class="rating_page_text" itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating">';
					$ratingValue = (floatval(number_format( $page_rate_rating, 2, '.', '' )) > 0.0) ? number_format( $page_rate_rating, 2, '.', '' ) : 1;
					$res .= '<span class="rating_page_text_rating" itemprop="ratingValue">'.$ratingValue.'</span>';
					$res .= ' / ';
...
<?php } else { ?>
    <li class="<?php echo $comment_class_print; ?> white_block" id="ajax-comment-<?php echo $comment->comment_ID; ?>"   data-commentid_v="13" data-commentid="<?php echo $comment->comment_ID; ?>"  data-postid="<?php echo $post_ID_comment; ?>" data-company_t="0" data-company="<?php echo get_field('company_name',$post_ID_comment); ?>"  data-url="<?php echo get_field('company_redirect_key',$post_ID_comment); ?>">
        ...
        <div class="comment-body body_<?php echo $comment->comment_ID; ?>" data-body-id="body_<?php echo $comment->comment_ID; ?>">
            <div class="comment_header flex">
                ...
                <div class="comment_meta">
                    ...
                    <div class="date-line"><span class="comment_to_id color_dark_gray font_small"></span> <span class="comment-date font_smaller color_dark_gray date_abuse" itemprop="datePublished" content="<?php comment_date( 'c', $comment->comment_ID ); ?>" data-profile="<?php comment_date('j F Y',$comment->comment_ID) ?>">
						<?php printf( __( '%s в %s', 'er_theme' ), get_comment_date( 'j F Y', $comment->comment_ID ), get_comment_date( 'H:i', $comment->comment_ID ) ); ?>
					</span></div>
                </div>
            ...
            <div class="comment_footer font_smaller">
				...
				if($comment_replies != 0) {
					?>
                    <div class="comment_reply_count dropdown flex color_dark_gray m_right_20 pointer comment_reply_count_get_abuse aaa"><?php _e('Развернуть','er_theme'); ?></div>
				<?php } ?>
                <div class="comment_footer_right hovered_<?php echo $comment->comment_ID; ?>">
		...
		<?php // без закрывающего </li> (!)
		if ($comment_replies != 0) {
			echo '<ul class="children visible" data-id="'.$commentid.'">';
		}
		...
		if(!function_exists('list_comments_ajax_widget')) {
			...
			function list_comments_ajax_widget() {
				...
				$post_ids_lang_string = implode(',', $posts_in_language->posts);
				...
				if ($comment_type && $comment_type != '') {
					if ($comment_type == 'reviews') {
						...
						$ext_where = " AND comment_ID IN ( SELECT comment_id FROM wp_commentmeta WHERE meta_key='comment_type' AND meta_value = 'reviews' ) ";

					} elseif ($comment_type == 'abuses') {

						$ext_where = " AND comment_ID IN ( SELECT comment_id FROM wp_commentmeta WHERE meta_key='is_abuse' AND meta_value = '1' ) ";
				...
				global $wpdb;
				$sql = "
					SELECT comment_ID, comment_date, comment_content, comment_post_ID, user_id
					FROM {$wpdb->comments} 
					WHERE comment_post_ID IN (". $post_ids_lang_string .") AND comment_approved = 1 " . $ext_where . "
				...
				if (!empty($comments)) {
					...
					foreach ($comments as $comment) {
						...
						if (!empty($rating_fields)) {
							$total_rating = floatval(get_comment_rating_values($rating_fields, $comment, 'total'));
							$total_rating_value = 5 / 100 * $total_rating;
							$data_percent = $total_rating / 100;
							if ($total_rating >= 70) {
								$rating_color = 'green';
						...
						if( !is_object( $comment_author ) ) {
							...
							$comment_author = new stdClass();
							$comment_author->user_nicename = 'Анонимный пользователь';
						...
						$result .= '<div class="comment_header">';
						$result .= '<div class="comment_avatar"';
						if ( isset( $attachment_id['sizes'] ) && $attachment_id['sizes']['thumbnail'] && $attachment_id['sizes']['thumbnail'] != '') {
		...
		if ( get_field( 'status_comment', $comment ) ) {
			...
			$showstatus = '<span class="set_status '.$set_status.'"><span class="status_comment_a"><span>'.get_field_object( 'status_comment', $comment )['choices'][ get_field( 'status_comment', $comment ) ].'</span></span></span>';
		} else {
			$set_status = '';
			$showstatus = '';
		}
							...
							$result .= '<div class="inner color_dark_blue font_bold font_small pointer total_rating_main_page v'.$set_status.'">' . round($total_rating_value, 1) . $showstatus.'</div>';
							$result .= '</div>';
						}
						...
						if ($comment_type == 'abuses') {
							$result .= ' ' . __('написал(а) жалобу на компанию', 'er_theme') . ' ';
						} else {
							$result .= ' ' . __('написал(а) отзыв к компании', 'er_theme') . ' ';
						}
						...
						if (mb_strpos($a, 'месяц') !== false) {
							$result .= ' <span data-id="44" class="comment-date">';
							if($current_language == 'fr_FR') {
								$result .= __('il y a ');
							}
							$result .= intval($a) . ' ' . counted_text($a, __('месяц', 'er_theme'), __('месяца', 'er_theme'), __('месяцев', 'er_theme')) . ' ';
							if($current_language != 'fr_FR') {
								$result .= __('назад', 'er_theme');
							}
							$result .= '</span>';
						} else {
							$result .= ' <span data-id="14"  class="comment-date">';
							if($current_language == 'fr_FR') {
								$result .= __('il y a ');
							} elseif ($current_language == 'de_DE') {
								$result .= __('vor ');
							...
							$result .= human_time_diff(strtotime(get_comment_date('Y-m-d H:i:s', $comment->comment_ID)), current_time('timestamp')).' ';
							if($current_language != 'fr_FR') {
								// if ($current_language == 'de_DE' || $current_language == 'es_ES' || $current_language == 'id_ID') {
								// 	//$result .= __('назад', 'er_theme');
								// } elseif ($current_language == 'en_US') {
								// 	$result .= __('ago');
								// } elseif ($current_language == 'pl_PL') {
						...
						$comm_content = $comment_data['content'];
						//}
						$result .= '<div data-id="1530" class="comment_text color_dark_blue do_not_translate_css_class">' . wp_trim_words($comm_content, $trim_number) . '</div>';
						$result .= '</div>';
						$result .= '</li>';
					}
					$result .= '</ul>';
			...
			function another_comm_1() {
				...
				if(!empty($comments_more)) {
					...
					foreach ($comments_more as $comm_more) {
						$comment_data = get_comment_current_content($comm_more->comment_ID);
						if(!$comment_data['available'] || $comment_data['available'] != 'yes') {
							echo '<li>';
							continue;
						}
						...
						$rating_fields = get_comment_rating_fields($rating_fields_group, 'key');
						if (!empty($rating_fields)) {
							$total_rating = floatval(get_comment_rating_values($rating_fields, $comm_more, 'total'));
							$total_rating_value = 5 / 100 * $total_rating;
							$data_percent = $total_rating / 100;
							if ($total_rating >= 70) {
								$rating_color = 'green';
						...
						if (mb_strpos($a, 'месяц') !== false) {
							$result .= ' <span data-id="44" class="comment-date do_not_translate_css_class">'.intval($a).' '.counted_text($a,'месяц','месяца','месяцев').' ';
							if($current_language != 'fr_FR') {
								$result .= __('назад', 'er_theme');
							}
							$result .= '</span>';
						} else {
							$result .= ' <span data-id="14"  class="comment-date do_not_translate_css_class">'.human_time_diff( strtotime(get_comment_date('Y-m-d H:i:s',$comm_more->comment_ID)), current_time( 'timestamp' ) ).' ';
							if($current_language != 'fr_FR') {
								$result .= __('назад', 'er_theme');
							}
							$result .= '</span>';
						...
						$comm_rev_title = $comment_data['title'];
						if(!$comm_rev_title || $comm_rev_title == '') {
							$comm_rev_title = wp_trim_words($comment_data['content'], 7,'');
						}
						...
						$result .= '<h3 class="comment_single_title do_not_translate_css_class">'.$comm_rev_title.'</h3>';
						
						$current_language = get_locale();
						if($current_language == 'ru_RU') {
							$text_read_review = 'Читать далее';
						}elseif($current_language == 'fr_FR') {
						...
						if ($current_language != 'ru_RU') {
							$comm_rev_text = wp_trim_words($comment_data['content'], $trim_number,'');
							/*if($current_language != 'ru_RU') {
							$translations = get_field('comment_translations','comment_'.$comm_more->comment_ID);
							//print_r($translations);
							...
							$comm_rev_text .= '...';
							$result .= '<div data-id="1401" class="comment_text color_dark_blue do_not_translate_css_class">' . $comm_rev_text . ' <a href="'.get_the_permalink($comm_more->comment_post_ID).'comment-'.$comm_more->comment_ID.'/" class="color_dark_gray">'.$text_read_review.'</a></div>';
						} else {
							$continue_reading = '<a href="'.get_the_permalink($comm_more->comment_post_ID).'comment-'.$comm_more->comment_ID.'/" class="continue_reading continue_1reading">'.$text_read_review.'</a>';
							$comm_rev_text = truncateToWord( $comment_data['content'], 300,$continue_reading );
							$result .= '<div data-id="1401" class="comment_text color_dark_blue do_not_translate_css_class">' . $comm_rev_text . '</div>';
						}
		...
		if(!function_exists('another_comm_1_print')) {
			...
			function another_comm_1_print($comment_post_ID,$comment__not_in,$current_language,$tex_more_rev_about,$company_name) {
				$result = '';
				/*$comment_post_ID = intval($_POST['comment_post_id']);
			$comment__not_in = intval($_POST['comment__not_in']);
			$current_language = $_POST['current_language'];
				...
				if(!empty($comments_more)) {
					...
					foreach ($comments_more as $comm_more) {
						...
						if ($current_language != 'ru_RU') {
							...
							$continue_reading = '<a href="'.get_the_permalink($comm_more->comment_post_ID).'comment-'.$comm_more->comment_ID.'/" class="continue_reading">'.$text_read_review.'</a>';
							$comm_rev_text = truncateToWord( $comment_data['content'], 300,$continue_reading );
							$result .= '<div data-id="1401" class="comment_text color_dark_blue do_not_translate_css_class">' . $comm_rev_text . '</div>';
						}
		...
		$da = get_comment_date( 'm-d-Y H:i', $comm_more->comment_ID );
		...
		// 	$da = intval($b).' '.counted_text($b,'месяц','месяца','месяцев').' ';
		// 	if($current_language != 'fr_FR') {
		// 		$da .= __('назад', 'er_theme');
		// 	}
		// 	$da .= '</span>';
		// } else {
		// 	$da = human_time_diff( strtotime(get_comment_date('Y-m-d H:i:s',$comm_more->comment_ID)), current_time( 'timestamp' ) ).' ';
		// 	if($current_language != 'fr_FR') {
		// 		$da .= __('назад', 'er_theme');
		// 	}
		// 	$da .= '</span>';
	...
	$result .= '<div class="comment_single_title comment_single_title_1 do_not_translate_css_class">'.$comm_rev_title.'</div>';
	...
	if ($current_language != 'ru_RU') {
		$comm_rev_text = $comment_data['content'];
		/*if($current_language != 'ru_RU') {
		$translations = get_field('comment_translations','comment_'.$comm_more->comment_ID);
		//print_r($translations);
		$comm_tr = $translations[0]['translation'];
		...
		$comm_rev_text .= '...';
		$result .= '<div data-id="7361" class="comment_text color_dark_blue do_not_translate_css_class">' . $comm_rev_text . ' <a href="'.get_the_permalink($comm_more->comment_post_ID).'comment-'.$comm_more->comment_ID.'/" class="color_dark_gray">'.$text_read_review.'</a></div>';
	} else {
		
		$continue_reading = '<a href="'.get_the_permalink($comm_more->comment_post_ID).'comment-'.$comm_more->comment_ID.'/" class="continue_reading continue_2reading">'.$text_read_review.'</a>';
		$comm_rev_text = truncateToWord( $comment_data['content'], 300,$continue_reading );
		$result .= '<div data-id="7361" class="comment_text color_dark_blue do_not_translate_css_class">' . $comm_rev_text . '</div>';
