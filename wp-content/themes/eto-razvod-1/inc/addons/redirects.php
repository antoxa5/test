		...
		if ( strpos('/'.$affkey_key[3], 'comment-') && !$affkey_key[4] ) {
			...
			if ( ! empty( $comment ) && $b_slug == $c_slug && $x_slug == 'review' && $comment->comment_approved == 1 ) {
				...
				wp_localize_script( 'jquery', 'banners_info', array(
					...
					'b_p_id'   => $comment->comment_post_ID
				) );
				set_query_var( 'comment_single_page', $comment );
				...
				//if (in_array($comment_ID, [ 24918, 47149, 49719, 49379, 49378, 36375, 34536 ])) {
				if (strtotime(get_comment_date('',$comment_ID)) > '1680998400' && $current_language == 'ru_RU') {
						...
						if ( $current_language != 'ru_RU' ) {
							...
							if ( $review_title && $review_title != '' ) {
								...
								$r_tit = wp_trim_words( $comment_data['content'], 7, '' );
							}
						}
						...
						if ( $current_language != 'ru_RU' ) {
							$company_name = str_replace('ранее ','',$company_name);
						...
						if ( $current_language != 'ru_RU' ) {
							...
							if ( $current_language == 'en_US' ) {
								$descr .= ' ' . __( '- the opinions of real people.', 'er_theme' ) . '';
							} elseif ( $current_language == 'de_DE' ) {
								$descr .= ' ' . __( '- die Meinungen von echten Menschen.', 'er_theme' ) . '';
							} elseif ( $current_language == 'es_ES' ) {
								$descr .= ' ' . __( '- las opiniones de personas reales.', 'er_theme' ) . '';
							} elseif ( $current_language == 'fr_FR' ) {
								$descr .= ' ' . __( '- les opinions de personnes réelles.', 'er_theme' ) . '';
							}
							...
							//$r_descr = wp_trim_words( wp_strip_all_tags($comment_data['content']), 18, '' );
							
							
							$r_descr = truncateToWord34(wp_strip_all_tags($comment_data['content']),160,'');
							$v = strlen($r_descr).$r_descr;
							
							$t1 = mb_strtolower(str_replace(array(' ', "\t", '.', ',', '!',"\r\n", "\r", "\n"), '', $comment_data['content']));
							$t2 = mb_strtolower(str_replace(array(' ', "\t", '.', ',', '!',"\r\n", "\r", "\n"), '', $r_descr));
							
							$desciption_string = mb_ucfirst( $r_descr, 'UTF-8' );
							$desciption_string = str_replace(["\r\n", "\r", "\n"],'',$desciption_string);
							
							if (mb_substr($desciption_string, -1) == ',') {
								$desciption_string = mb_substr( $desciption_string, 0, - 1 );
							}
						...
						$r_tit_temp = mb_strtolower(str_replace(array(' ', "\t", '.', ',', '!',), '', $r_tit));
						$descr_temp = mb_strtolower(str_replace(array(' ', "\t", '.', ',', '!',), '', $descr));
						if (mb_strlen($descr) < 90) {
							$descr_add = ' - Реальный отзыв о '.get_field('company_name', $comment->comment_post_ID);
						} else {
							$descr_add = '';
						}
						
						if (mb_strlen($comm_rev_title) < 50) {
							if (str_contains(strtolower($comm_rev_title), 'рекомендую')) {
								$comm_rev_title_add = '';
							} else {
								if (get_field( 'review_recommend_friends', 'comment_' . $comment_ID )) {
									if (get_field( 'review_recommend_friends', 'comment_' . $comment_ID ) == 'yes') {
										$comm_rev_title_add = '';
									} else {
										$comm_rev_title_add = '';
									}
						...
						$comm_rev_title = mb_ucfirst( $comm_rev_title, 'UTF-8' );
						/*$check = preg_match( '/(\.|\?|\!)/', mb_substr( $descr, - 1 ) );
						if ( $check != 1 ) {
							$descr = $descr.'.';
						}*/
						...
						$comm_rev_title = str_replace( ' &quot;', ' «', $comm_rev_title );
						$comm_rev_title = str_replace( '&quot; ', '» ', $comm_rev_title );
						$comm_rev_title = str_replace( '&quot;.', '».', $comm_rev_title );
						set_query_var( 'page_title', $comm_rev_title.$comm_rev_title_add.' | Это развод™' );
						
						$descr = str_replace( ' &quot;', ' «', $descr );
						$descr = str_replace( '&quot; ', '» ', $descr );
						$descr = str_replace( '&quot;.', '».', $descr );
						set_query_var( 'page_description', $descr.$descr_add );
...
add_action('init', 'set_dashboard_adv_company');

function set_dashboard_adv_company () {
	global $wpdb;
	global $current_user;
	...
	if (isset($_GET['affkey'])) {
		$request = '/dashboard/adv/company/'.$_GET['affkey'].'/';
	}
	
	
	if ( strpos('/'.$request, '/dashboard/adv/company/') ) {
		$affkey_key = explode('dashboard/adv/company/', $request);
		$affkey_key = $affkey_key[1];
		$affkey_key = str_replace('/', '', $affkey_key);
		if($affkey_key) {
			//get_page_by_path( $affkey_key, 'ARRAY_A', 'casino' )['ID']
			if (get_page_by_path( $affkey_key, 'ARRAY_A', 'casino' ) ) {
				...
				if ($current_user_id == $get_user_id) {
					...
					wp_localize_script( 'jquery', 'company_page',
						array( 'user_id' =>  $get_users[0]->data->ID,'company_id' => $idPage,'company_slug' => $affkey_key));
					get_template_part( 'user', 'company_adv' );
					exit;
				}
			}
...
function room($affkey_key) {
	...
	if ( have_posts() ) {
		...
		$items = get_field('company_site_affiliate_url_lang',$post->ID);
