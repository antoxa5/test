		...
		if($format == 'layout') {
			if ( $fields ) {
				...
				if($get_ratings_count['count'] > 0) {
					...
					$result .= '
							<span class="inner color_dark_blue font_bold font_20" itemprop="ratingValue">' . $print_total_round . '</span>
	';
...
//        }
		...
		$comments_top .= '<div class="comments_top_title font_uppercase font_bolder color_dark_blue font_smaller_2">'.__('Отзывы','er_theme').'</div>';
		$comments_top .= '<div class="comments_top_count font_bold color_dark_gray font_small">'.$get_ratings_count['count'].'</div>';
		if ($user_id == "undefined") {
			$comments_top .= '<div class="link_review_popup button button_comments button_green pointer font_small font_bold">' . __( 'Оставить отзыв', 'er_theme' ) . '</div>';
		}
		if ($data['slug'] == 'comments') {
...
if(!function_exists('ajax_abuses')) {
	...
	function ajax_abuses() {
		...
		$comments_top .= '<div class="comments_top_count font_bold color_dark_gray font_small">'.count($comments_sorted).'</div>';
		if (($sort_type != 'user') && ($user_id == "undefined")) {
...
function custom_comment_single_hidden( $comment, $args, $depth){
	if($depth > 1) { ?>
        <li  id="show-comment-<?php comment_ID(); ?>">
            ...
            <div class="comment_text"><?php if ($comment->user_id == 31) {
        ...
        <li id="show-comment-<?php echo $comment->comment_ID; ?>" itemprop="review" itemscope itemtype="http://schema.org/Review" data-commentid="<?php comment_ID(); ?>">
			...
			<?php };
			?>
            <span class="comment-date" itemprop="datePublished" content="<?php comment_date( 'Y-m-d',$comment->comment_ID ); ?>"><?php comment_date( 'j F Y в H:i',$comment->comment_ID ); ?></span>
            <div itemprop="reviewBody" class="comment_text comment_text_<?php echo $comment->comment_ID; ?>"><?php if ($comment->user_id == 31) {
	                        $textfrompost = $comment->comment_content;
	                        if ( strstr( $textfrompost, '<a href' ) ) {
		                        $textfull = $comment->comment_content;
	                        } else {
...
if ($commentid != NULL) { ?>
    <li class="<?php echo $comment_class_print; ?> white_block" id="comment-<?php echo $comment->comment_ID; ?>" data-commentid="<?php comment_ID(); ?>"  data-postid="<?php echo $post_ID_comment; ?>" data-company="<?php echo get_field('company_name',$post_ID_comment); ?>"  data-url="<?php echo get_field('company_redirect_key',$post_ID_comment); ?>">
		<?php } else { ?>
        <li class="<?php echo $comment_class_print; ?> white_block" id="comment-<?php echo $comment->comment_ID; ?>" data-commentid="<?php comment_ID(); ?>"  data-postid="<?php echo $post_ID_comment; ?>" data-company="<?php echo get_field('company_name',$post_ID_comment); ?>"  data-url="<?php echo get_field('company_redirect_key',$post_ID_comment); ?>">
			<?php } ?>
            <div class="comment-body body_<?php echo $comment->comment_ID; ?>" data-body-id="body_<?php echo $comment->comment_ID; ?>">
                <div class="comment_header flex">
                    ...
                    <div class="comment_meta">
                        <div class="comment_author_wrapper m_b_5">
							<?php $comment_author = get_userdata( $comment->user_id ); ?>
                            <span class="comment-author font_bold font_small color_dark_blue pointer" itemprop="author" data-link="<?php echo get_bloginfo('url').'/user/'.$comment_author->user_nicename.'/'; ?>" <?php echo $pubprofile; ?>><?php if($comment_author->first_name && !$comment_author->last_name) { echo $comment_author->first_name; } elseif(!$comment_author->first_name && $comment_author->last_name) { echo $comment_author->last_name; } elseif($comment_author->first_name && $comment_author->last_name) { echo $comment_author->first_name.' '.$comment_author->last_name; } else { echo $comment_author->user_nicename; } ?></span>
							<?php echo $labelpro; ?><?php echo $user_status_label; ?>
                        </div>
                        <div class="date-line"><span class="comment_to_id color_dark_gray font_small"></span> <span class="comment-date font_smaller color_dark_gray date_abuse" itemprop="datePublished" content="<?php comment_date( 'Y-m-d',$comment->comment_ID ); ?>" data-profile="<?php comment_date('j F Y',$comment->comment_ID) ?>"><?php comment_date( 'j F Y в H:i',$comment->comment_ID ); ?></span></div>
                    </div>
                ...
                <div class="comment_footer font_smaller">
                    ...
                    <div class="comment_fast_links hovered_<?php echo $comment->comment_ID; ?>">
                        ...
                        <span class="comment_more_actions_link_spam"><?php _e('Пожаловаться','er_theme'); ?></span>
        ...
        <li class="<?php echo $comment_class_print; ?>" id="comment-<?php comment_ID(); ?>" data-commentid="<?php echo $comment->comment_ID; ?>">
            <div class="comment-body body_<?php echo $comment->comment_ID; ?>" data-body-id="body_<?php echo $comment->comment_ID; ?>">
                <div class="comment_header flex">
                    ...
                    <div class="comment_meta">
                    <span class="comment-author font_bold font_small color_dark_blue m_b_5" <?php echo $pubprofile; ?> data-link="<?php echo get_bloginfo('url').'/user/'.$comment_author->user_nicename.'/'; ?>"><?php if($comment_author->first_name && !$comment_author->last_name) { echo $comment_author->first_name.' '.$labelpro; } elseif(!$comment_author->first_name && $comment_author->last_name) { echo $comment_author->last_name.' '.$labelpro; } elseif($comment_author->first_name && $comment_author->last_name) { echo $comment_author->first_name.' '.$comment_author->last_name.' '.$labelpro; } else { echo $comment_author->user_nicename.' '.$labelpro; } ?><?php echo $user_status_label; ?><span class="comment_parent_author_link font_normal color_dark_gray">
...
<?php if($par_aurhor->first_name && !$par_aurhor->last_name) { echo $par_aurhor->first_name; } elseif(!$par_aurhor->first_name && $par_aurhor->last_name) { echo $par_aurhor->last_name; } elseif($par_aurhor->first_name && $par_aurhor->last_name) { echo $par_aurhor->first_name.' '.$par_aurhor->last_name; } else { echo $par_aurhor->user_nicename; } ?>
                ...
                <div class="comment_footer font_smaller">
                    ...
                    <a rel="nofollow" class="comment-reply-link color_dark_blue pointer inactive" data-commentid="<?php echo $comment->comment_parent ?>" data-postid="<?php echo $comment->comment_post_ID; ?>" data-appendto="<?php echo $comment->comment_ID; ?>"><?php _e('Ответить','er_theme'); ?></a>
                    <div class="comment_footer_right hovered_<?php echo $comment->comment_ID; ?>">
			...
			$post_ID_comment = $comment->comment_post_ID;?>
        <li class="<?php echo $comment_class_print; ?> white_block" id="comment-<?php echo $comment->comment_ID; ?>" itemprop="review" itemscope itemtype="http://schema.org/Review" data-commentid="<?php comment_ID(); ?>" data-postid="<?php echo $post_ID_comment; ?>" data-company="<?php echo get_field('company_name',$post_ID_comment); ?>"  data-url="<?php echo get_field('company_redirect_key',$post_ID_comment); ?>">
            <meta itemprop="itemReviewed" content="<?php echo get_field('company_name' ); ?>" />
            <div class="comment-body body_<?php echo $comment->comment_ID; ?>" data-body-id="body_<?php echo $comment->comment_ID; ?>">
                <div class="comment_header flex">
					<?php $comment_author = get_userdata( $comment->user_id ); ?>
                    ...
                    <div class="comment_meta">
                        <div class="comment_author_wrapper m_b_5">
							<?php $comment_author = get_userdata( $comment->user_id ); ?>
                            <span class="comment-author font_bold font_small color_dark_blue pointer" itemprop="author" data-link="<?php echo get_bloginfo('url').'/user/'.$comment_author->user_nicename.'/'; ?>" <?php echo $pubprofile; ?>><?php if($comment_author->first_name && !$comment_author->last_name) { echo $comment_author->first_name; } elseif(!$comment_author->first_name && $comment_author->last_name) { echo $comment_author->last_name; } elseif($comment_author->first_name && $comment_author->last_name) { echo $comment_author->first_name.' '.$comment_author->last_name; } else { echo $comment_author->user_nicename; } ?></span>
							<?php echo $labelpro; ?><?php echo $user_status_label; ?> <span class="comment_to_id color_dark_gray"></span>
                        </div>
                        <span class="comment-date font_smaller color_dark_gray" itemprop="datePublished" content="<?php comment_date( 'Y-m-d',$comment->comment_ID ); ?>"><?php comment_date( 'j F Y в H:i',$comment->comment_ID ); ?></span>
                    </div>
                ...
                <div class="comment_footer font_smaller">
					...
					<?php } ?>
                    <a rel="nofollow" class="comment-reply-link color_dark_blue pointer inactive" data-commentid="<?php echo $comment->comment_ID; ?>" data-postid="<?php echo $comment->comment_post_ID; ?>" data-appendto="<?php comment_ID(); ?>"><?php _e('Ответить','er_theme'); ?></a>
                    <div class="comment_footer_right hovered_<?php echo $comment->comment_ID; ?>">
...
//			}
		...
		function custom_comment_single_profile_noanswears(  $comment = NULL, $args = NULL, $depth = NULL,$commentid = NULL){
			...
			function custom_comment_single_profile( $comment, $args, $depth){
        ...
        <li class="<?php echo $comment_class_print; ?>" id="comment-<?php comment_ID(); ?>" data-commentid="<?php echo $comment->comment_ID; ?>">
            <div class="comment-body body_<?php echo $comment->comment_ID; ?>"  data-body-id="body_<?php echo $comment->comment_ID; ?>"  data-parentid="<?php echo $comment->comment_post_ID; ?>"  data-post-status="<?php echo get_post_status($comment->comment_post_ID); ?>" data-body-id="body_<?php echo $comment->comment_ID; ?>">
                <div class="comment_header">
                    ...
                    <div class="comment_meta">
                    <span class="comment-author" <?php echo $pubprofile; ?> data-link="<?php echo get_bloginfo('url').'/user/'.$comment_author->user_nicename.'/'; ?>"><?php if($comment_author->first_name && !$comment_author->last_name) { echo $comment_author->first_name.' '.$labelpro; } elseif(!$comment_author->first_name && $comment_author->last_name) { echo $comment_author->last_name.' '.$labelpro; } elseif($comment_author->first_name && $comment_author->last_name) { echo $comment_author->first_name.' '.$comment_author->last_name.' '.$labelpro; } else { echo $comment_author->user_nicename.' '.$labelpro; } ?><span class="comment_parent_author_link font_normal color_dark_gray">
<?php $par_comm_obj = get_comment( $comment->comment_parent );
$par_aurhor = get_userdata($par_comm_obj->user_id);
...
<?php if($par_aurhor->first_name && !$par_aurhor->last_name) { echo $par_aurhor->first_name; } elseif(!$par_aurhor->first_name && $par_aurhor->last_name) { echo $par_aurhor->last_name; } elseif($par_aurhor->first_name && $par_aurhor->last_name) { echo $par_aurhor->first_name.' '.$par_aurhor->last_name; } else { echo $par_aurhor->user_nicename; } ?>
                ...
                <div class="comment-content">
                    <div class="comment_text comment_text_<?php echo $comment->comment_ID; ?>"><?php if ($comment->user_id == 31) {
	                        $textfrompost = $comment->comment_content;
	                        if ( strstr( $textfrompost, '<a href' ) ) {
		                        $textfull = $comment->comment_content;
	                        } else {
                ...
                <div class="comment_footer">
                    ...
                    <a rel="nofollow" style="display:none" class="comment-reply-link" data-commentid="<?php echo $comment->comment_parent ?>" data-postid="<?php echo $comment->comment_post_ID; ?>" data-appendto="<?php echo $comment->comment_ID; ?>">Ответить</a>
                    <div class="comment_footer_right hovered_<?php echo $comment->comment_ID; ?>">
        ...
        <li class="<?php echo $comment_class_print; ?>" id="comment-<?php echo $comment->comment_ID; ?>" itemprop="review" itemscope itemtype="http://schema.org/Review" data-commentid="<?php comment_ID(); ?>">
            ...
            <div class="comment-body body_<?php echo $comment->comment_ID; ?>"  data-body-id="body_<?php echo $comment->comment_ID; ?>"  data-parentid="<?php echo $comment->comment_post_ID; ?>"  data-post-status="<?php echo get_post_status($comment->comment_post_ID); ?>" data-body-id="body_<?php echo $comment->comment_ID; ?>">
                <div class="comment_header">
                    ...
                    <div class="comment_meta">
                        <div class="comment_author_wrapper m_b_5">
							<?php $comment_author = get_userdata( $comment->user_id ); ?>
                            <span class="comment-author pointer" itemprop="author" data-link="<?php echo get_bloginfo('url').'/user/'.$comment_author->user_nicename.'/'; ?>" <?php echo $pubprofile; ?>><?php if($comment_author->first_name && !$comment_author->last_name) { echo $comment_author->first_name; } elseif(!$comment_author->first_name && $comment_author->last_name) { echo $comment_author->last_name; } elseif($comment_author->first_name && $comment_author->last_name) { echo $comment_author->first_name.' '.$comment_author->last_name; } else { echo $comment_author->user_nicename; } ?></span>
							<?php echo $labelpro; ?>
                        </div>
                        <span class="comment-date" itemprop="datePublished" content="<?php comment_date( 'Y-m-d',$comment->comment_ID ); ?>"><?php comment_date( 'j F Y в H:i',$comment->comment_ID ); ?></span>
                    </div>
            ...
            <?php } */?>
                    <a rel="nofollow" class="comment-reply-link" data-commentid="<?php echo $comment->comment_ID; ?>" style="display:none" data-postid="<?php echo $comment->comment_post_ID; ?>" data-appendto="<?php comment_ID(); ?>">Ответить</a>
                    <div class="comment_footer_right hovered_<?php echo $comment->comment_ID; ?>">
