	...
	function resort_table() {
		...
		/*if ( ! empty( $fields ) ) {
			foreach ( $fields as $field ) {
				...
				if ($key == 'filter_special_freelance') {
	...
	function rating_table_more( $tag, $query, $offset, $post_id ) {
		...
		if ( $reviews->have_posts() ) {
			...
			while ( $reviews->have_posts() ) {
				...
				$result .= '<div class="main_row rating_table_row font_smaller" id="row_' . $i . '" data-menu-order="'.$post->menu_order.'" data-q="1">';
	...
	function rating_table( $tag, $query ) {
		...
		if ( $reviews->have_posts() ) {
			...
			while ( $reviews->have_posts() ) {
				...
				$result .= '<div class="main_row rating_table_row font_smaller" id="row_' . $i . '" data-menu-order="'.$post->menu_order.'" data-row-id="'.$i.'" data-tags="'.implode(",", get_field('review_aff_tags',get_the_ID())).'" data-q="2">';
				$result .= '<div class="rating_table_td rating_field_number font_bold">' . $i . '</div>';
				foreach ( $row_1 as $row ) {
					$ffvalue_0 = '';
					$ffvalue_0 = brokers_field_new( $row, get_the_ID(), $the_field );
					if ( $ffvalue_0 && $ffvalue_0 != '' && $ffvalue_0 != 'N/A' ) {
						$result .= '<div class="rating_table_td ' . $row['class'] . ' rating_field_' . $row['value']['value'] . '" data-a="1">' . brokers_field_new( $row, get_the_ID(), $the_field ) . '</div>';
					} else {
						$result .= '<div class="rating_table_td ' . $row['class'] . ' rating_field_' . $row['value']['value'] . '" data-a="2">' . $row['text_na'] . '</div>';
					}
...
function brokers_field_new( $item, $post_id, $the_field ) {
	...
	if ( $field_id == 'link_review' ) {
		...
		$result       .= '<div class="company_title line_big" data-id="'.$post_id.' '.wp_get_attachment_image_url(get_post_meta($post_id,'company_logo',true),'large').'">';
