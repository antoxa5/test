...
if ( ! function_exists( 'resort_table' ) ) {
	...
	function resort_table() {
        ...
        if(!empty($data['filter_fields'])) {
            //print_r($data['filter_fields']);
            //sa_print
            $new_meta_query =  sa_convert_filter_fields($data['filter_fields'],$args);
            if(!empty($new_meta_query['meta_query'])) {
                $args['meta_query'][] = $new_meta_query['meta_query'];
            }
            if(!empty($new_meta_query['post__in'])) {
                $args['post__in'] = $new_meta_query['post__in'];
            }
        }
		$reviews = new WP_Query( $args );
		if ( $current_language != 'ru_RU' ) {
...
if ( ! function_exists( 'rating_table_default' ) ) {
	function rating_table_default( $tag, $fields ) {
        ...
        if(!empty($reviews->query['meta_query'])) {
            $jsoned_query = json_encode($reviews->query['meta_query']);
            echo '
		<script type=\'text/javascript\'>
			var current_query = \''.$jsoned_query.'\';
			jQuery(document).ready(function($){
			//alert(JSON.stringify(current_query));
			$("#sa_rating_filter_form").ready(function() {
			';

            echo '$("#sa_rating_filter_form").append(\'<input name="prefiltered" type="hidden" value="'.htmlspecialchars($jsoned_query).'" />\'); ';
...
if ( ! function_exists( 'rating_table' ) ) {
	function rating_table( $tag, $query, $sorted = 1 ) {
		...
		$reviews = $query;
        $rating_default_data = array();
        $args3 = $reviews->query;
        $args3['posts_per_page'] = '-1';
        $reviews_get_fields = new WP_Query( $args3 );
        if ( $reviews_get_fields->have_posts() ) {
            while ( $reviews_get_fields->have_posts() ) {
                $reviews_get_fields->the_post();
                $company_established = '';
                $company_established = get_field('company_established',get_the_ID());
                if($company_established && $company_established != '') {
                    $rating_default_data['company_established'][] = $company_established;
                }
                $reviews_count_reviews = '';
                $reviews_count_reviews = get_field('reviews_count_reviews',get_the_ID());
                if($reviews_count_reviews && $reviews_count_reviews != '') {
                    $rating_default_data['reviews_count_reviews'][] = $reviews_count_reviews;
                }

            }
        }
        //print_r($rating_default_data);
        $predefined_values = array();
        if(!empty($rating_default_data)) {
            foreach($rating_default_data as $key => $value) {
                $array_date_min = min($value);

                $array_date_max = max($value);

                if($array_date_min == 1) {
                    $array_date_min = 0;
                }
                $predefined_values[$key.'-min'] = $array_date_min;
                $predefined_values[$key.'-max'] = $array_date_max;
            }
        }

        //print_r($predefined_values);
		if ( $reviews->have_posts() ) {
			$count = $reviews->found_posts;
            if ($courses_special == 1) {
            ...
            if(!empty($predefined_values)) {
                foreach($predefined_values as $key => $value) {
                    $result .= 'dataglobal-'.$key.'="'.$value.'"';
                }
            }
            $result .= '>';
...
function brokers_field_new( $item, $post_id, $the_field, $courses_special = 0, $real_id = 0 ) {
	...
	if ( $field_id == 'link_review' ) {
	...
	} elseif ( $field_id == 'best_casino_bonuses' ) {
		if (gettype(get_field('base_2_bonuses_first_dep_best_444', $post_id)) == 'array') {
			if (count(get_field('base_2_bonuses_first_dep_best_444', $post_id)) > 0) {

				$result_text = '';
				$i_base_2_bonuses_first_dep_best_444 = 0;
				foreach ( get_field('base_2_bonuses_first_dep_best_444', $post_id) as $item ) {
					++$i_base_2_bonuses_first_dep_best_444;
					if ($i_base_2_bonuses_first_dep_best_444 == count(get_field('base_2_bonuses_first_dep_best_444', $post_id))) {
	...
	} elseif ( $field_id == 'realesedate' ) {
		$item = get_field( 'withdrawal_time_2', $post_id );
		if ($item) {
			$result .= '<span class="">'.get_field( 'withdrawal_time_2', $post_id ).'</span>';
		}
		
	} elseif ( $field_id == 'referral-program' ) {
		$item = get_field( 'affiliate_program_referals_0_link', $post_id );
		if ($item) {
			$result .= '<a class="field_link" href="'.str_replace("https://etorazvod.ru","",$item).'" target="_blank" rel="nofollow">Open</a>';
		}
	...
	} elseif ( $field_id == 'countryslots' ) {
		$item = get_field( 'countries_and_regions', $post_id );
		if ($item) {
			$i = 0;
			foreach ($item  as $item3 ) {
				$i = ++$i;
				if (count($item) == $i) {
					$result .= get_term_by( 'id', $item3, 'regions' )->name.'';
				} else {
					$result .= get_term_by( 'id', $item3, 'regions' )->name.', ';
				}
	...
	} elseif ( $field_id == 'genreslots' ) {
		$item = get_field( 'game_genre', $post_id );
		if ($item) {
			$i = 0;
			foreach ($item  as $item3 ) {
				$i = ++$i;
				if (count($item) == $i) {
					$result .= get_term_by( 'id', $item3, 'gamegenres' )->name.'';
				} else {
					$result .= get_term_by( 'id', $item3, 'gamegenres' )->name.', ';
				}
	...
	} elseif ( $field_id == 'slotsbonus' ) {
		$item = get_field( 'pros_4', $post_id );
		if ($item) {
			$i = 0;
			foreach ($item  as $item3 ) {
				$i = ++$i;
		...
		$item = get_field( 'loan_approval_percentage_rtp', $post_id )[0];
		if ($item) {
	...
	} elseif ( $field_id == 'casinoforslots' ) {
		$item = get_field( 'been_casino', $post_id );
		$i = 0;
		foreach ($item  as $item3 ) {
			$i = ++$i;
			if (count($item) == $i) {
				$result .= '<a href="'.get_the_permalink($item3).'">'.get_field('company_name',$item3).'</a>';
			} else {
				$result .= '<a href="'.get_the_permalink($item3).'">'.get_field('company_name',$item3).'</a>, ';
			}
			
		}
	} elseif ( $field_id == 'referalslots' ) {
		$item = get_field( 'affiliate_program_0_link', $post_id );
		if ($item) {
			$result .= '<a href="'.get_field( 'affiliate_program_0_link', $post_id ).'">'.__('Registration','sa_theme').'</a>';
		}
		...
		if ($item) {
			$result .= '<a class="field_link link_c_table" href="'.str_replace("https://etorazvod.ru","",get_field( 'free_demo_games_0_link', $post_id )).'" target="_blank" rel="nofollow">Open a demo account</a>';
		}
	...
	} elseif ( $field_id == 'slotsdev' ) {
		$item = get_field_object( 'casino_software2', $post_id );
		if ( simple_field( $item['type'], $item, 'rating' ) ) {
			$result .= simple_field( $item['type'], $item, 'rating' );
		} else {
			$result .= $na;
	...
	} elseif ( $field_id == 'slotsdemo' ||  $field_id == 'best_casino_bonuses_demo') {
		$item = get_field( 'free_demo_games_2_slots_0_link', $post_id );
		if ( get_field( 'free_demo_games_2_slots_0_link', $post_id ) ) {
			$result .= '<div class="btn_link_f"><a href="'.get_field( 'free_demo_games_2_slots_0_link', $post_id ).'">'.__('Free demo game','sa_theme').'</a></div>';
		} else {
			$result .= $na;
		}
...
if(!function_exists('sa_rating_filters_field_display')) {
    function sa_rating_filters_field_display($field, $settings) {
        //echo 'function_field: <pre>';
        //print_r($field);
        //echo '</pre>';
        //echo 'function_settings: <pre>';
        //print_r($settings);
        //echo '</pre>';
        $result = '';
        if($field['filter_name'] && $field['filter_name'] != '') {
            $field_title = $field['filter_name'];
        } else {
            $field_title = $field['filter_field']->name;
        }

        if(in_array($settings['type_of_field'],array('checkbox_in','checkbox','taxonomy','number_more','mobile','age','taxonomy_multiple', 'repeater_multi'))) {
            $result .= '<div class="filter_field type_checkbox_in checkbox_container font_smaller color_dark_blue">';
            $result .= '<input name="filter_fields[' . $field['filter_field']->term_id . ']" type="checkbox" class="custom-checkbox custom-checkbox-green" id="filter_field_' . $field['filter_field']->term_id . '" />';
            $result .= '<label for="filter_field_' . $field['filter_field']->term_id . '">' . $field_title . '</label>';
            $result .= '</div>';
        } elseif($settings['type_of_field'] == 'date_between') {
            $result .= '<div class="filter_field type_date_between" data-target-field-name="'.$settings['filter_field_name'].'">';
            $result .= '<div class="font_smaller color_dark_blue">'.$field_title.'</div>';
            $result .= '<div class="range_container m_t_10" id="range_'.$field['filter_field']->term_id.'">';
            $result .= '<div class="sliders_control">';
            $result .= '<input id="fromSlider" type="range" value="" min="0" max="'.date("Y").'"/>';
            $result .= '<input id="toSlider" type="range" value="" min="0" max="'.date("Y").'"/>';
            $result .= '</div>';
            $result .= '<div class="form_control">';
            $result .= '<div class="form_control_container flex flex_wrap range_from">';
            $result .= '<div class="form_control_container__time font_smaller m_right_10 color_dark_gray">'.__('От','er_theme').'</div>';
            $result .= '<input class="form_control_container__time__input" type="number" name="filter_fields[' . $field['filter_field']->term_id . '][from]" id="fromInput" value="" min="" max=""/>';
            $result .= '</div>';
            $result .= '<div class="form_control_container flex flex_wrap range_to">';
            $result .= '<div class="form_control_container__time font_smaller m_right_10 color_dark_gray">'.__('До','er_theme').'</div>';
            $result .= '<input class="form_control_container__time__input" type="number" name="filter_fields[' . $field['filter_field']->term_id . '][to]" id="toInput" value="" min="" max=""/>';
            $result .= '</div>';
            $result .= '</div>';
            $result .= '</div>';
            $result .= '</div>';

        } elseif($settings['type_of_field'] == 'text') {
            $result .= '<div class="m_b_10 filter_field type_text"><input class="input_small" type="text" name="filter_fields[' . $field['filter_field']->term_id . ']" value="" placeholder="'.$field_title.'" ></div>';
        } else {
            $result .= '<div>'.$field_title.'</div>';
        }
...
if(!function_exists('sa_rating_filters')) {
    function sa_rating_filters($tag) {
        //echo get_the_ID();
        //sa_print
        $result = '';
        $result_fields = '';
        if (!empty($tag)) {
            $filter_fields = get_field('tag_filters','term_'.$tag->term_id);
            $field_group_id = 0;

            if(empty($filter_fields)) {
                $filter_fields = get_field('tag_filters','term_1065');
            }
            //print_r($filter_fields);
            if(!empty($filter_fields)) {
                foreach($filter_fields as $filter_field_group) {
                    $field_group_id++;
                    if($filter_field_group['filter_group_name'] && $filter_field_group['filter_group_name'] != '') {
                        $result_fields .= '<div class="field_group m_b_15" id="field_group_'.$field_group_id.'">';
                        if(!$filter_field_group['filter_group_name_hide']) {
                            $result_fields .= '<div class="font_bold color_dark_blue m_b_10 font_small">'.$filter_field_group['filter_group_name'].'</div>';
                        }
                        if(!empty($filter_field_group['filter_group'])) {
                            foreach($filter_field_group['filter_group'] as $filter_field) {
                                if(!empty($filter_field['filter_field'])) {
                                    $filter_field_settings = get_fields('term_'.$filter_field['filter_field']->term_id);
                                    if(empty($filter_field_settings)) {
                                        $filter_field_settings = array(
                                            'type_of_field' => 'custom'
                                        );
                                    } elseif(!$filter_field_settings['type_of_field'] || $filter_field_settings['type_of_field'] == '') {
                                        $filter_field_settings['type_of_field'] = 'custom';
                                    }
                                    $field_layout = sa_rating_filters_field_display($filter_field, $filter_field_settings);
                                    $result_fields .= $field_layout;
                                }
            ...
            $result .= '<div class="sa_rating_filter background_light block_closed" id="rating_fillter_container">';
            $result .= '<div class="wrap flex_column p_t_10 p_bottom_zero">';
            $result .= '<span class="sa_rating_filter_close pointer font_smaller dropdown flex color_dark_blue m_b_10" data-text-open="'.__('Показать фильтр','er_theme').'" data-text-close="'.__('Скрыть фильтр','er_theme').'">'.__('Скрыть фильтр','er_theme').'</span>';
            $result .= '<div class="form_show_hide">';
            $result .= '<form class="p_30 flex flex_row flex_wrap inactive_form white_block rating_filter_form" id="sa_rating_filter_form" action="'.admin_url( 'admin-ajax.php' ).'" method="post">';
            $result .= '<input type="hidden" name="action" value="sa_rating_filter_form_submit" />';
            $result .= '<input type="hidden" name="tag" value="'.$tag->slug.'" />';
            //$result .= $tag->term_id.' '.$tag->slug;
            //$result .= '<div class="acf-input"><div class="acf-input-wrap"><input type="text" name="filter_fields[company_name]" value="" ></div></div>';
            $result .= $result_fields;
            $result .= '<div class="acf-form-submit flex flex_wrap flex_100">';
            $result .= '<span class="sa_rating_filter_reset pointer font_smaller color_dark_blue m_l_auto m_right_20">'.__('Сбросить фильтр','er_theme').'</span>';
            $result .= '<input type="submit" class="button button_gray pointer button-large" value="'.__('Фильтровать','er_theme').'">';
            $result .= '</div>';
            $result .= '</form>';
            $result .= '</div>';
            $result .= '</div>';
...
function my_posts_where_filter_mobile( $where ) {

    $where = str_replace("meta_key = 'base_2_mobile_version_app_links_$", "meta_key LIKE 'base_2_mobile_version_app_links_%", $where);

    return $where;
}

add_filter('posts_where', 'my_posts_where_filter_mobile');


if(!function_exists('sa_convert_filter_fields')) {
    function sa_convert_filter_fields($fields = array(), $default_args = array()) {
        //sa_print print_r($fields);
        $found_ids = array();
        $meta_query = array();
        ...
        if(!empty($fields)) {
            ...
            foreach($fields as $field_name => $value) {
                if($field_name == 'company_name') {
                    ...
                    //echo 'field_query_name:'.$field_name.'<br/>';
                    $field_array = array();
                    $field_array = get_fields('term_'.$field_name);

                    if(
                        $field_array['type_of_field'] && $field_array['type_of_field'] != ''
                        //&& $field_array['filter_field_name'] && $field_array['filter_field_name'] != ''
                        //&& $field_array['filter_field_value'] && $field_array['filter_field_value'] != ''
                    ) {
                        if($field_array['type_of_field'] == 'number_more') {
                            $meta_query[] = array(
                                'key' => $field_array['filter_field_name'],
                                'value' => $field_array['filter_field_value'],
                                'compare' => '>='
                            );
                        } elseif($field_array['type_of_field'] == 'checkbox') {
                            $meta_query[] = array(
                                'key' => $field_array['filter_field_name'],
                                'value' => $field_array['filter_field_value'],
                                'compare' => '='
                            );
                        } elseif($field_array['type_of_field'] == 'repeater_multi') {
                           // echo 'test';
                            $repeater_multi_fields = $field_array['repeater_multi_fields'];
                            foreach($repeater_multi_fields as $repeater_multi_field) {

                                $default_args['meta_query'] = array(
                                    'relation' => 'AND',
                                );
                                foreach($repeater_multi_field['items'] as $item) {
                                    $default_args['meta_query'][] = array(
                                        'key' => $item['filter_field_name_repeater'],
                                        'value' => $item['filter_field_value_repeater'],
                                        'compare' => $item['filter_field_operator_repeater'],
                                        'type' => $item['filter_field_compare_type_repeater'],
                                    );
                                }
                                $look_for_ids = new WP_Query( $default_args );
                                if(!empty($look_for_ids->posts)) {
                                    foreach ($look_for_ids->posts as $post) {
                                        $found_ids[] = $post->ID;
                                    }
                                }
                                wp_reset_postdata();
                                //echo 'every_field<pre>';
                               // print_r($default_args);
                               // echo '</pre>';
                            }
                        } elseif($field_array['type_of_field'] == 'age') {
                            $meta_query[] = array(
                                'key' => $field_array['filter_field_name'],
                                'value' => $field_array['filter_field_value'],
                                'compare' => $field_array['filter_field_operator'],
                                'type'    => 'NUMERIC'
                            );
                        } elseif($field_array['type_of_field'] == 'checkbox_in') {
                            $meta_query[] = array(
                                'key' => $field_array['filter_field_name'],
                                'value' => sprintf('"%s"', $field_array['filter_field_value']),
                                'compare' => 'LIKE'
                            );
                        } elseif($field_array['type_of_field'] == 'taxonomy') {
                            $meta_query[] = array(
                                'key' => $field_array['filter_field_name'],
                                'value' => ':"'.$field_array['filter_field_value'].'";',
                                'compare' => 'LIKE'
                            );
						} elseif($field_array['type_of_field'] == 'taxonomy_multiple') {
							$terms = array();
							$terms = explode(",", $field_array['filter_field_value']);
							// print_r($terms);
							if(!empty($terms)) {
								$push_terms = array(
									'relation' => 'OR',
								...
								$meta_query[] = $push_terms;
							}
                        } elseif($field_array['type_of_field'] == 'mobile') {
                            $meta_query[] = array(
                                'key' => $field_array['filter_field_name'],
                                'value' => $field_array['filter_field_value'],
                                'compare' => 'LIKE'
                            );
                        } elseif($field_array['type_of_field'] == 'date_between') {

                            if($field_array['filter_field_name'] == 'company_established') {
                                /* Эта сволочь-дата если даже ее не заполнить,
                                все равно фильтрует как-то по-своему*/
                                $meta_query[] = array(
                                    'key' => $field_array['filter_field_name'],
                                    'value' => $value['from'],
                                    'compare' => '>=',
                        ...
                        elseif($field_array['type_of_field'] == 'text') {
                            $meta_query[] = array(
                                'key' => $field_array['filter_field_name'],
                                //'value' => ':"'.$field_array['filter_field_value'].'";',
                                'value' => $value,
        ...
        if(!empty($meta_query)) {
            ...
            foreach($meta_query as $meta) {
                $meta_query_result[] = $meta;
            }
        }
        if(!empty($meta_query_result)) {
            $result['meta_query'] = $meta_query_result;
        }
        if(!empty($found_ids)) {
            $found_ids = array_unique($found_ids);
           // echo 'found_ids';
           // print_r($found_ids);
            $result['post__in'] = $found_ids;
        }
        //echo 'return_result:<pre>';
        //print_r($result);
       // echo '</pre>';
...
if ( ! function_exists( 'sa_rating_filter_form_submit' ) ) {
    add_action('wp_ajax_sa_rating_filter_form_submit', 'sa_rating_filter_form_submit');
    add_action('wp_ajax_nopriv_sa_rating_filter_form_submit', 'sa_rating_filter_form_submit');
    function sa_rating_filter_form_submit() {
        $data = $_POST;
        ...
        if(!empty($data['filter_fields'])) {
            $meta_query =
                sa_convert_filter_fields($data['filter_fields'],$args);
            if(!empty($meta_query['meta_query'])) {
                $args['meta_query'] = $meta_query['meta_query'];
            }
            if(!empty($meta_query['post__in'])) {
                $args['post__in'] = $meta_query['post__in'];
            }

        }

        if($data['prefiltered'] && !$data['prefiltered'] == '') {
            $prefiltered_query = json_decode(wp_unslash($data['prefiltered']),true);
            //echo '<pre>';
            //print_r($prefiltered_query);
            //echo '</pre>';
            //sa_print
            foreach ($prefiltered_query as $prefiltered_single) {
                if(is_array($prefiltered_single)) {
                    $args['meta_query'][] = $prefiltered_single;
                    //echo '<pre>';
                    //print_r($prefiltered_single);
                    //echo '</pre>';
                    //sa_print
                }
        ...
        //sa_print
        $reviews = new WP_Query( $args );
        $result_table = rating_table( $tag, $reviews );
        if($result_table == '') {
            $result .= '<p class="color_dark_blue text_centered">'.__('По выбранным параметрам ничего не найдено, измените критерии поиска и повторите попытку.','er_theme').'</p>';
        } else {
            $result  .= $result_table;
        }
