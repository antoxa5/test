		...
		if ( $reviews->have_posts() ) {
			...
			while ( $reviews->have_posts() ) {
				if (in_array(get_the_ID(), $arr_post_temp)) {
					...
					$result .= '<div class="main_row rating_table_row font_smaller" id="row_' . $i . '" data-menu-order="'.$post->menu_order.'" data-b="'.implode(',',get_field('enable_translations',get_the_ID())).'">';
		...
		if ( $reviews->have_posts() ) {
			...
			while ( $reviews->have_posts() ) {
				...
				if (in_array(get_the_ID(), $arr_post_temp)) {
					...
					$result .= '<div class="main_row rating_table_row font_smaller" id="row_' . $i . '" data-menu-order="'.$post->menu_order.'" data-row-id="'.$i.'" data-t="3" data-tags="'.implode(",", get_field('review_aff_tags',get_the_ID())).'" data-b="'.implode(',',get_field('enable_translations',get_the_ID())).'">';
					$result .= '<div class="rating_table_td rating_field_number font_bold">' . $i . '</div>';
					foreach ( $row_1 as $row ) {
						$ffvalue_0 = '';
						$ffvalue_0 = brokers_field_new( $row, get_the_ID(), $the_field );
