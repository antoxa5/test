...
if ( ! function_exists( 'er_js_connect' ) ) {
	function er_js_connect() {
		...
		wp_enqueue_script( 'first', get_template_directory_uri() . '/js/first.js', array( 'jquery' ), $special_word.filemtime( TEMPLATEPATH . '/js/first.js' ) );
				...
				if (get_locale() == 'ru_RU') {
					...
					if ($data['tech_notify3']) {
						...
						if (intval($data['tech_notify3']) == 1) {
							...
							$url = 'https://api.dashamail.ru/?method=lists.add_member&&api_key=e9c51614cdd55d1e810e44e5499d8d77&list_id=145821&email='.$data['email'].'&format=xml';
...
function restrict_comments_page_access() {
	...
	if ( $screen->base === 'edit-comments' && $current_user->data->ID == 33652 ) {
...
/*function hide_admin_wpse_93843() {
	if (current_user_can('editor_plus')) {
...
function add_toolbar_items($admin_bar){
	...
	if ($wp_get_current_user->data->ID != 33652 ) {
