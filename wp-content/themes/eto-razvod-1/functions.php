...
function er_include_addons() {
	...
	include_once( TEMPLATEPATH . '/inc/promocode_advcake.php' );
	...
	include_once( TEMPLATEPATH . '/inc/addons/test_payments.php' );
        include_once( TEMPLATEPATH . '/inc/addons/generate_widgets/widget_raring_small.php' );
}


add_action( 'wp_enqueue_scripts', 'er_enqueue_styles' );
function er_enqueue_styles() {
wp_enqueue_style( 'style', get_template_directory_uri() . '/style.css', '', '1.10.464'  );
}


if ( ! function_exists( 'live_search' ) ) {
	function live_search( $id, $pageid = 0 ) {
		$result = '';
		//if ($pageid == 154471) {
			$result .= '<link itemprop="url" href="'.get_home_url().'"/><form method="post" action="' . admin_url( 'admin-ajax.php' ) . '" class="live_search inactive_live_search" id="' . $id . '" itemprop="potentialAction" itemscope itemtype="http://schema.org/SearchAction">';
			$result .= '<meta itemprop="target" content="'.get_home_url().'/search/?q={s}"/><input itemprop="query-input" type="text" name="s" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" aria-label="'.__('Искать', 'er_theme').'" />';
...
if ( ! function_exists( 'print_css_links' ) ) {
	function print_css_links( $data ) {
		...
		$links['adv']                 = '<link rel="stylesheet" id="adv" href="' . get_template_directory_uri() . '/css/adv.css?ver=' . $special_word.filemtime( TEMPLATEPATH . '/css/adv.css' ) . '" type="text/css" media="all" />';
...
if ( ! function_exists( 'fast_links' ) ) {
	function fast_links( $id ) {
		...
		$result           .= '</div>';
		$result           .= '<div class="fast_links_content wrap" style="display: none">';
		//$result           .= '<div class="wrap">';
		...
		//if ( ! in_array( $current_language, array( 'en_US', 'fr_FR', 'es_ES', 'de_DE' ) ) ) {
			...
			if(strpos(theme_nav( 'fast_links_1', 1, 'fast_1', 'fast_1', 'link_dark_blue font_smaller' ), '_item_') !== false) {
				$result .= '<div class="fast_link_item nav">' . theme_nav( 'fast_links_1', 1, 'fast_1', 'fast_1', 'link_dark_blue font_smaller' ) . '</div>';
			}
			if(strpos(theme_nav( 'fast_links_2', 1, 'fast_2', 'fast_2', 'link_dark_blue font_smaller' ), '_item_') !== false) {
				$result .= '<div class="fast_link_item nav">' . theme_nav( 'fast_links_2', 1, 'fast_2', 'fast_2', 'link_dark_blue font_smaller' ) . '</div>';
			}
			if(strpos(theme_nav( 'fast_links_3', 1, 'fast_3', 'fast_3', 'link_dark_blue font_smaller' ), '_item_') !== false) {
				$result .= '<div class="fast_link_item nav">' . theme_nav( 'fast_links_3', 1, 'fast_3', 'fast_3', 'link_dark_blue font_smaller' ) . '</div>';
			}
			if(strpos(theme_nav( 'fast_links_4', 1, 'fast_4', 'fast_4', 'link_dark_blue font_smaller' ), '_item_') !== false) {
				$result .= '<div class="fast_link_item nav">' . theme_nav( 'fast_links_4', 1, 'fast_4', 'fast_4', 'link_dark_blue font_smaller' ) . '</div>';
			}
			if(strpos(theme_nav( 'fast_links_5', 1, 'fast_5', 'fast_5', 'link_dark_blue font_smaller' ), '_item_') !== false) {
				$result .= '<div class="fast_link_item nav">' . theme_nav( 'fast_links_5', 1, 'fast_5', 'fast_5', 'link_dark_blue font_smaller' ) . '</div>';
			}
			if(strpos(theme_nav( 'fast_links_6', 1, 'fast_6', 'fast_6', 'link_dark_blue font_smaller' ), '_item_') !== false) {
				$result .= '<div class="fast_link_item nav">' . theme_nav( 'fast_links_6', 1, 'fast_6', 'fast_6', 'link_dark_blue font_smaller' ) . '</div>';
			}
			
			//}
			
			if(strpos(theme_nav( 'fast_links_7', 1, 'fast_7', 'fast_7', 'link_dark_blue font_smaller' ), '_item_') !== false) {
				$result .= '<div class="fast_link_item nav">' . theme_nav( 'fast_links_7', 1, 'fast_7', 'fast_7', 'link_dark_blue font_smaller' ) . '</div>';
			}
			
			if(strpos(theme_nav( 'fast_links_8', 1, 'fast_8', 'fast_8', 'link_dark_blue font_smaller' ), '_item_') !== false) {
				$result .= '<div class="fast_link_item nav">' . theme_nav( 'fast_links_8', 1, 'fast_8', 'fast_8', 'link_dark_blue font_smaller' ) . '</div>';
			}
			//if ( ! in_array( $current_language, array( 'en_US', 'fr_FR', 'es_ES', 'de_DE' ) ) ) {
			if(strpos(theme_nav( 'fast_links_9', 1, 'fast_9', 'fast_9', 'link_dark_blue font_smaller' ), '_item_') !== false) {
				$result .= '<div class="fast_link_item nav">' . theme_nav( 'fast_links_9', 1, 'fast_9', 'fast_9', 'link_dark_blue font_smaller' ) . '</div>';
			}
			...
			if(strpos(theme_nav( 'fast_links_10', 1, 'fast_10', 'fast_10', 'link_dark_blue font_smaller' ), '_item_') !== false) {
				$result .= '<div class="fast_link_item nav">' . theme_nav( 'fast_links_10', 1, 'fast_10', 'fast_10', 'link_dark_blue font_smaller' ) . '</div>';
			}
	...
	function review_main( $post_id, $type = '' ) {
		...
		if ( get_post_type() == 'promocodes' ) {
			...
			if ( $company_title && $company_title != '' ) {
				$result .= '<h1 class="review_company_title review_company_title_1 font_bold font_medium_new color_dark_blue">' . $company_title . '</h1>';
			}
		} else {
			if ( $company_title && $company_title != '' ) {
				$current_language = get_locale();
				...
				if ( $content && $content != '' ) {
					if (preg_match('/[А-Яа-яЁё]/u', $company_title) == 1) {
						$result .= '<div class="review_company_title review_company_title_3 font_bold font_medium_new color_dark_blue">' . $company_title . '</div>';
					} else {
						$result .= '<div class="review_company_title review_company_title_3 font_bold font_medium_new color_dark_blue do_not_translate_css_class">' . $company_title . '</div>';
					}
					
				} else {
					if ( $type == 'single_review_page' ) {
						$result .= '<div class="review_company_title font_bold font_medium_new color_dark_blue link_no_underline">' . $company_title . '</div>';
					} else {
						if (preg_match('/[А-Яа-яЁё]/u', $company_title) == 1) {
							$result .= '<h1 class="review_company_title review_company_title_2 font_bold font_medium_new color_dark_blue">' . $company_title . '</h1>';
						} else {
							$result .= '<h1 class="review_company_title review_company_title_2 font_bold font_medium_new color_dark_blue do_not_translate_css_class">' . $company_title . '</h1>';
						}
					}
		...
		if (get_post_type() == 'promocodes') {
			$result .= '<div class="stars_and_reviews stars_and_reviews_2 flex">';
		} else {
			if( $reviews_count > 0 ) {
				$result .= '<div class="stars_and_reviews stars_and_reviews_2 flex" itemscope="" itemprop="aggregateRating" itemtype="https://schema.org/AggregateRating">';
			} else {
				$result .= '<div class="stars_and_reviews stars_and_reviews_2 flex">';
			}
		}
...
if ( ! function_exists( 'review_top_rating' ) ) {
	function review_top_rating( $post_id, $font_big = true, $type = '' ) {
		...
		if ( $type == 'singletest' ) {
			...
			$result1 .= '<span class="inner color_dark_blue font_bold font_20" itemprop="ratingValue">' . $ratingValue . '</span>';
	...
	function review_card( $post_id ) {
		...
		if ( $verified || $er_company_site_str && $er_company_site_str != '' ) {
			...
			if ( $er_company_site_str && $er_company_site_str != '' ) {
				...
				$result .= '<div class="site_url font_bold color_dark_blue font_22' . $icon_insta . '" target="_blank" data-no-translation>' . $n_url . '</div>';
...
if ( ! function_exists( 'review_logo' ) ) {
	function review_logo( $post_id, $lazy = false, $linked = false, $webp = false ) {
		...
		if ( $linked == true ) {
			...
			$result .= '<a itemprop="url" class="review_logo '.$verifed.'" href="' . get_the_permalink( $post_id ) . '"';
		} else {
			if (get_field( 'company_verified_status', $post_id )) {
		...
		if ( $linked == true ) {
			$result .= ' aria-label="Logo"><meta itemprop="logo" content="' . $logo['sizes']['large'] . '"></a>';
		} else {
			$result .= '></div>';
		};
...
if ( ! function_exists( 'review_top' ) ) {
	function review_top( $post_id, $type = '' ) {
		...
		$result .= '<div class="review_header" itemscope="" itemtype="http://schema.org/Organization">';
		$result .= '<div class="wrap">';
...
if ( ! function_exists( 'page_top' ) ) {
	function page_top( $post_id, $type ) {
		...
		if ( $type == 'rating' ) {
			...
			wp_reset_postdata();
			if( ( is_array($arrayPosts) || $arrayPosts instanceof Countable ) && ( count( $arrayPosts ) != 0 ) ) {
				$arr_temp  = [];
				$args_temp = [
				...
				$args_temp['alt_pages_hide'] = 1;

				// $childrens = get_children( $args_temp );
				$childrens = new WP_Query( $args_temp );
...
if ( ! function_exists( 'review_bar' ) ) {
	function review_bar( $post_id = 0 ) {
		...
		$result .= '<a class="review_icon_redirect pointer" href="' . $link . '" target="_blank" aria-label="Visit link"></a>';
		$result .= '</div>';
...
if ( ! function_exists( 'review_links' ) ) {
	function review_links( $post_id = 0 ) {
		...
		if ( $promocodes_active == true ) {
			...
			$result .= '<li><a href="' . get_the_permalink( $post_id ) . '#about" class="color_dark_gray link_no_underline" term-id="'.$term_id.'">' . $tab_company . '</a></li>';
			
			$result .= '<li><a href="' . get_the_permalink( $post_id ) . '#comments" class="color_dark_gray link_no_underline">' . __( 'Отзывы', 'er_theme' ) . '</a></li>';
			if ( $content_2 != '' ) {
				if (get_field('tab_custom_review', 'term_' . $term_id)) {
					if (!empty(get_field('tab_custom_review', 'term_' . $term_id))) {
						$full_reviwie = get_field('tab_custom_review', 'term_' . $term_id);
					} else {
						$full_reviwie = __( 'Обзор', 'er_theme' );
					}
				} else {
					$full_reviwie = __( 'Обзор', 'er_theme' );
				}
				$result .= '<li><a href="' . get_the_permalink( $post_id ) . '#fullreview" class="color_dark_gray link_no_underline">' . $full_reviwie . '</a></li>';
			}
			...
			$result .= '<li class="review_link_company pointer ' . $company_active . '" data-tab="review_container_about"><a href="' . get_the_permalink( $post_id ) . '#about" class="color_dark_gray link_no_underline"  term-id="'.$term_id.'">' . $tab_company . '</a></li>';
			...
			if ( $content != '' || $term_slug == 'cryptocurrency' ) {
				...
				$result .= '<li class="review_link_fullreview pointer" data-tab="review_container_content"><a href="' . get_the_permalink( $post_id ) . '#fullreview" class="color_dark_gray link_no_underline">' . $full_reviwie . '</a></li>';
			}
			...
			while ( $promocodes_query->have_posts() ) {
				...
				if ( $current_language != 'ru_RU' ) {
					...
					// print_r($is_enabled);
					if ( is_array( $is_enabled ) && in_array( $current_language, $is_enabled ) ) {
	...
	function user_nav_settings() {
		...
		if ( count( $company_ids2 ) != 0 ) {
			...
			foreach ( $company_ids2 as $item ) {
				...
				$result  .= '<a href="' . $link . '" class="review_logo" aria-label="'.get_field( 'company_name', $post_id ).'"';
				$logo    = get_field( 'company_logo', $post_id );
				$logo_bg = get_field( 'company_icon_bg', $post_id );
				if ( $logo_bg && $logo_bg != '' ) {
					$bg = ' background-color:' . $logo_bg . ';';
...
//					}
			...
			if($data['post_type'] == 'promocodes') {
				...
				foreach($promocodes_array as $promocode_single) {
					...
					$result .= '<a class="promocode_item_title color_dark_blue link_no_underline font_bold" href="' . get_the_permalink($post->ID) . '" target="_blank">' . $company_name . '</a>';
					if ( $item['title'] != '' ) {
		...
		if ( is_post_type_archive( array( 'promocodes' ) ) ) {
			...
			if ( is_singular( 'promocodes' ) ) {
				...
				if ($current_language == 'en_US') {
					...
					$result = str_replace('промокоды', "tarjouskoodit", $result);
					$result = str_replace('промокод', "tarjouskoodi", $result);
					$result = str_replace('промо коды', "tarjouskoodit", $result);
					$result = str_replace('промо код', "tarjouskoodi", $result);
					$result = str_replace('промо-коды', "tarjouskoodit", $result);
					$result = str_replace('промо-код', "tarjouskoodi", $result);
				} elseif ($current_language == 'id_ID') {
					$result = str_replace('промокоды', "kode promo", $result);
					$result = str_replace('промокод', "kode promo", $result);
					$result = str_replace('промо коды', "kode promo", $result);
					$result = str_replace('промо код', "kode promo", $result);
					$result = str_replace('промо-коды', "kode promo", $result);
					$result = str_replace('промо-код', "kode promo", $result);
				}
				...
				if ( $tag_add != '' ) {
					...
					if ( $title && $title != '' && $title_priority == true ) {
						...
						if ( $current_language == 'en_US' ) {
							...
							$get_title = str_replace( 'это развод?', "", $get_title );
							$get_title = str_replace( $cyr, $lat, $get_title );
							$get_title .= 'onko kyseessä petos?';
						} elseif ( $current_language == 'id_ID' ) {
							$get_title = str_replace( 'это развод?', "", $get_title );
							$get_title = str_replace( $cyr, $lat, $get_title );
							$get_title .= 'apakah ini penipuan?';
						}
...
// Подменяем post_id для записей, у которых есть альтернативная версия ACF:alt_pages
function alter_the_query( $request ) {
	...
	// Если у страницы есть альтернативная страница на другом языке,
	// то меняем эту страницу на альтернативную
	if( isset( $request['pagename'] ) or ( isset( $request['name'] ) and isset( $request['post_type'] ) and in_array( $request['post_type'], ['casino', 'promocodes', 'promocodes_cats', 'addpages'] ) ) ) {

		$page_name = isset( $request['pagename'] ) ? $request['pagename'] : $request['name'];

		// Для страниц с родителем берем из имени страницы значение после последнего слеша
		if( strpos( $page_name, '/' ) !== false ) {

			$page_name = end( explode( '/', $page_name ) );
		...
		$meta = $wpdb->get_results( $wpdb->prepare( "
			...
			AND LOWER(p.post_name)=LOWER('%s')
			AND pm.meta_key='alt_pages' 
			LIMIT 0,1
		", $page_name ) );

		// Если есть ACF-поле Повторитель для всех установленных языков, то ищем ссылки для текущего языка среди дочерних элементов Повторителя найденного поста
		if( isset( $meta[0]->ID ) AND isset( $meta[0]->post_type ) AND in_array( $meta[0]->post_type, ['page', 'post', 'casino', 'promocodes', 'promocodes_cats', 'addpages'] ) ) {

			$pre = $wpdb->remove_placeholder_escape( $wpdb->prepare( "
				SELECT pm.meta_key, pm.meta_value
				FROM wp_postmeta AS pm
				WHERE pm.post_id='%d' AND
				pm.meta_key LIKE '%s'
			", $meta[0]->ID, 'alt_pages_%' ) );
			$meta = $wpdb->get_results( $pre );

		}

		if( count( $meta ) ) {

			// С помощью деструктуризации переформатируем плоский список ключ-значение в двумерный массив пар пост-язык
			foreach( $meta as $val ) {
				list( ,,$index,,, $field ) = explode( '_', $val->meta_key );
				$alts[$index][$field] = $val->meta_value;
			}
		...
		if( count( $alts ) ) {
			...
			foreach( $alts as $val ) {
				if( isset( $val['id'] ) and $val['id'] and isset( $val['language'] ) and $val['language'] == $curr_language ) {

					unset( $request['pagename'] );
					unset( $request['name'] );
					$request['page_id'] = $val['id'];
...
// TODO
// при сохранении поста с выбранной альтернативной страницей проверять, нет ли уже других постов с указанной такой же альтернативной страницей

	/**
	* Если для этого урла не нашли альтернативную страницу, то ищем, есть ли страница, для которой текущая страница 
	* установлена альтернативной и делаем редирект на адрес этой исходной страницы
	*/
	if( $page_name ) {

		$pre = $wpdb->remove_placeholder_escape( $wpdb->prepare( "
			SELECT p2.ID, p2.post_name, p2.post_type, pm2.meta_value
			FROM wp_posts AS p, wp_postmeta AS pm 
			LEFT JOIN wp_posts AS p2 ON pm.post_id=p2.ID 
			LEFT JOIN wp_postmeta AS pm2 ON pm.post_id=pm2.post_id AND pm2.meta_key LIKE '%s'
			...
			LIMIT 0,1
		", 'alt_pages_%_alt_page_language', $page_name, 'alt_pages_%_alt_page_id' ) );
		$meta = $wpdb->get_results( $pre );

		if( isset( $meta[0]->ID ) and isset( $meta[0]->meta_value ) and $meta[0]->meta_value == $curr_language ) {

			if( isset( $meta[0]->post_name ) and isset( $meta[0]->post_type ) and $meta[0]->post_type == 'addpages' ) {

				$review_id = get_field( 'addpage_review', $meta[0]->ID );
				$review_url = get_post_permalink( $review_id );

				wp_redirect( $review_url . $meta[0]->post_name, 301 );
				exit;

			}

			if( isset( $meta[0]->post_name ) and isset( $meta[0]->post_type ) and $meta[0]->post_type == 'promocodes' ) {

				$terms_promocode_taxonomy = get_term( get_field( 'promocode_taxonomy', $meta[0]->ID ), 'affiliate-tags' );
				...
				if( isset( $terms_promocode_taxonomy->slug ) ) {
					...
					if ( $terms ) {
						...
						wp_redirect( str_replace( '%show_category%', $slug_url, get_permalink( $meta[0]->ID ) ), 301 );
						exit;
			...
			if( isset( $meta[0]->ID ) ) {
				...
				wp_redirect( get_permalink( $meta[0]->ID ), 301 );
				exit;
...
/*
	...
	if( isset( $wp_query->get_queried_object()->post_name ) and isset( $wp_query->get_queried_object()->post_type ) and $wp_query->get_queried_object()->post_type == 'page' ) {
		...
		$reassign = array(
			...
			'fi' => array(
				'legal' => '228702',
				'privacy-policy' => '228708',
				'rules' => '228711',
				'terms-of-use' => '228714',
				'offer' => '228717',
				'personal-data' => '228724',
			),
			'id_ID' => array(
				'legal' => '229686',
				'privacy-policy' => '229719',
				'rules' => '229722',
				'terms-of-use' => '229725',
				'offer' => '229728',
				'personal-data' => '229732',
			),
		);
...
function custom_front_page( $wp_query ) {
	...
	if ( $wp_query->get( 'page_id' ) == get_option( 'page_on_front' ) ):
		if ( $curr_language == 'en_US' ) {
			...
			$wp_query->set( 'page_id', 228694 );
		} elseif ( $curr_language == 'id_ID' ) {
			$wp_query->set( 'page_id', 229680 );
		} else {
		...
		if ( is_singular() ) {
			...
			if ( $redirect_on_language && $redirect_on_language != 'no' ) {
				...
				$locales = [
					...
					'fi' => [
						'uri' => '/home-fi/',
						'link' => 'https://fi.revieweek.com',
					],
					'id_ID' => [
						'uri' => '/home-id/',
						'link' => 'https://id.revieweek.com',
					],
				];

				foreach( $locales as $locale_key => $locale ) {

					if ( $redirect_on_language == $locale_key ) {

						if ( $_SERVER['REQUEST_URI'] == $locales[$locale_key]['uri'] ) {

							wp_redirect( $locales[$locale_key]['link'] );
						exit;
...
function show_banners_id_wp_admin( $post_id, $banner_place ) {
	...
	if ( ! empty( $banners ) ) {
		...
		foreach ( $banners as $post_id ) {
			$x ++;
			$result .= '<div>';
			$result .= '<a href="https://etorazvod.ru/wp-admin/post.php?post=' . $post_id . '&action=edit" target="_blank">';
...
function retrivemailnewfunc() {
	...
	if ( filter_var( $retrivemail, FILTER_VALIDATE_EMAIL ) ) {
		...
		if ( email_exists( $retrivemail ) ) {
			...
			// $message .= 'Если Вы не запрашивали восстановление пароля - то не переходите по данной ссылке. <img src="https://etorazvod.ru/engine/mail_update_status.php?key=' . $mail_key . '" style="width:1px; height:1px;" />';
			$message .= __( 'Если Вы не запрашивали восстановление пароля - то не переходите по этой ссылке.', 'er_theme' ) . ' <img src="https://etorazvod.ru/engine/mail_update_status.php?key=' . $mail_key . '" style="width:1px; height:1px;" />';
...
function admin_head_post_editing() {
	...
	if ( ( in_array( $current_user->data->ID, $arr_ids ) ) || ( in_array( 'author', $current_user->roles ) ) ) {
		//echo 'editing_2';
		if ( ( ( $post->post_status == 'publish' ) && ( get_field( 'company_type', $post->ID ) != 15604 ) && ( ! ( in_array( $post->ID, $arr_posts ) ) ) ) && ( $post->post_author != $current_user->data->ID && get_post_meta( $post->ID, '_edit_last', true ) != $current_user->data->ID ) ) {
			//echo 'editing_3';
			//print_r($post);
			if ($post->post_author != 34351) {
...
function my_admin_head_new_user() {
	global $current_user;
	if ( $current_user->data->ID == 34570 ) {
		echo '<style>li#menu-posts, li#menu-media, li#menu-pages,li#menu-comments, li#menu-posts-shortcoder, li#menu-posts-banners,li#menu-posts-promocodes,li#menu-posts-promocodes,li#menu-posts-addpages,li#menu-posts-promocodes_cats {
    display: none !important;
}

li#menu-posts-casino > ul > li:nth-of-type(3) ~ li {
    display: none;
}
...
add_action( 'admin_head', 'my_admin_head_new_user' );


function restrict_comments_page_access() {
	$screen = get_current_screen();
	global $current_user;
	
	if ( $screen->base === 'edit-comments' && ($current_user->data->ID == 34570) ) {
		wp_die( 'You are not allowed to access this page.' );
	}
}
add_action( 'current_screen', 'restrict_comments_page_access' );
...
function disable_comment_editing_deleting_for_user_id_9( $actions, $comment ) {
	global $current_user;
	if ( ($current_user->data->ID == 34570) ) {
		unset( $actions['edit'] );
...
add_filter( 'comment_row_actions', 'disable_comment_editing_deleting_for_user_id_9', 10, 2 );
...
function exclude_ru( $query ) {
	...
	if ( $query->is_archive ) {
		...
		if ( $current_language == 'ru_RU' ) {
			$query->set( 'turn_off_on_ru_language', 1 );
		}

	}

}
add_action( 'pre_get_posts', 'exclude_ru' );
...
function er_posts_filter( $clauses, $query ){
	...
	// Фильтруем записи, у которых есть альтернативные страницы на текущем языке
	if ( ( 1 === $query->get( 'parent_alt_pages_hide' ) ) ) {
		
		$clauses['where'] .= " AND wp_posts.ID NOT IN ( SELECT post_id FROM wp_postmeta WHERE meta_key LIKE 'alt_pages_%_alt_page_language' AND meta_value = '".$current_language."' ) ";

	}

	// Скрываем альтернативные страницы на текущем языке
	if ( ( 1 === $query->get( 'alt_pages_hide' ) ) ) {
		
		$clauses['where'] .= " AND wp_posts.ID NOT IN ( 
			SELECT DISTINCT pm.meta_value
			FROM wp_postmeta AS pm 
			LEFT JOIN wp_postmeta AS pm2 ON pm.post_id=pm2.post_id AND pm2.meta_key LIKE 'alt_pages_%_alt_page_language'
			WHERE pm.meta_key LIKE 'alt_pages_%_alt_page_id'
			AND pm2.meta_value!='".$current_language."'
			AND pm.meta_value!=''
		) ";
...
function get_crypto_buying() {
	...
	// $link = '<div class="topreview cta_full"><a class="cta" href="https://etorazvod.ru/visit/binance-coin-buy/" target="_blank" rel="noopener">Купить ' . get_field( 'company_name', get_the_ID() ) . ' на Binance</a></div>';
	$link = '<div class="topreview cta_full"><a class="cta" href="https://etorazvod.ru/visit/binance-coin-buy/" target="_blank" rel="noopener">' . wp_sprintf( __( 'Купить %s на Binance', 'er_theme' ), get_field( 'company_name', get_the_ID() ) ) . '</a></div>';
...
if ( ! function_exists( 'replace_ap_fb_rev_link' ) ) {
	function replace_ap_fb_rev_link( $text ) {
		...
		if ( $current_language == 'en_US' ) {
			...
			$text = str_replace( 'eto-razvod.ru', 'fi.revieweek.com', $text );
			$text = str_replace( 'Это развод', 'Revieweek', $text );
			$text = str_replace( '[sc name="social"][/sc]', '', $text );
			$text = str_replace( '[sc name="social"]', '', $text );
			$text = str_replace( '[/sc]', '', $text );
		} elseif ( $current_language == 'id_ID' ) {
			$text = str_replace( 'eto-razvod.ru', 'id.revieweek.com', $text );
			$text = str_replace( 'Это развод', 'Revieweek', $text );
			$text = str_replace( '[sc name="social"][/sc]', '', $text );
			$text = str_replace( '[sc name="social"]', '', $text );
			$text = str_replace( '[/sc]', '', $text );
...
// <br><br>
// Правила пользования сервисом <a href="https://etorazvod.ru/rules/">https://etorazvod.ru/rules/</a>.<br><br>
...
', 'er_theme' ), $aa, '<a href="'.get_permalink($get_comment->comment_post_ID).'comment-'.$get_comment->comment_ID.'/">'.$review_title.'</a>' );
				...
				if (get_post_type($get_comment->comment_post_ID) == 'casino' && intval($get_comment->comment_parent) == 0) {
					...
					if (mb_strlen($get_comment->comment_author_email) > 0 && is_email( $get_comment->comment_author_email ) && mb_strpos($get_comment->comment_author_email, 'eto.ru') === false) {
						if (get_field('from_site_send', 'user_'.$get_comment->user_id)) {
							if (intval(get_field('from_site_send', 'user_'.$get_comment->user_id)) != 0) {
...
add_action( 'after_setup_theme', 'special_word' );
...
if ( ! function_exists( 'truncateToWord34' ) ) {
	function truncateToWord34( $content, $length = 200, $continue_reading = '' ) {
		if ( mb_strlen( $content ) >= $length ) {
			if (mb_strpos( $content, '" ', $length ) == mb_strpos( $content, ' ', $length )-1 ){
				return $content;
			}
...
function allchargeback( $atts ){
	$language = get_locale();
	if ($language == 'ru_RU') {
		return '<iframe class="nes-iframe" src="https://allchargebacks.org/partners/forms/main/index.php?utm_source=partner333667" frameborder="0"></iframe>
<link rel="stylesheet" href="https://allchargebacks.org/partners/forms/main/iframe.css">';
	} else {
		return '';
	}
}
...
function add_toolbar_items($admin_bar){
	...
	if (	( $wp_get_current_user->data->ID != 34570) ) {
		...
		if (((get_post_type(get_the_ID()) == 'casino') || (get_post_type(get_the_ID()) == 'post') || (get_post_type(get_the_ID()) == 'page'))  && get_locale() == 'en_US') {
		...
		if (((get_post_type(get_the_ID()) == 'casino') || (get_post_type(get_the_ID()) == 'post') || (get_post_type(get_the_ID()) == 'page'))  && get_locale() == 'en_US') {
			...
			$admin_bar->add_menu( array(
				'id'    => 'my-third-sub-item3',
				'parent' => 'my-item',
				'title' => 'Перевести EN => FI',
				'href'  => "http://".$_SERVER['HTTP_HOST'].$url."?go_translate_fi=1",
				'meta'  => array(
					'title' => __('My Second Sub Menu Item aa'),
					'target' => '_blank',
					'class' => 'my_menu_item_class3'
				),
			));
		}
		...
		if (((get_post_type(get_the_ID()) == 'casino') || (get_post_type(get_the_ID()) == 'post') || (get_post_type(get_the_ID()) == 'page'))  && get_locale() == 'en_US') {
			...
			$admin_bar->add_menu( array(
				'id'    => 'my-third-sub-item4',
				'parent' => 'my-item',
				'title' => 'Перевести EN => ID',
				'href'  => "http://".$_SERVER['HTTP_HOST'].$url."?go_translate_id=1",
				'meta'  => array(
					'title' => __('My Second Sub Menu Item aa'),
					'target' => '_blank',
					'class' => 'my_menu_item_class4'
				),
			));
		}
	} else {
		$admin_bar->add_menu( array(
			...
			'href'  => '/wp-admin/edit.php?post_type=redirects',
			'meta'  => array(
				'title' => __('Перейти в админ-панель'),
			),
		));
	}
	
	
	/*if (((get_post_type(get_the_ID()) == 'casino') || (get_post_type(get_the_ID()) == 'post') || (get_post_type(get_the_ID()) == 'page'))  && get_locale() == 'pl_PL') {
		
		$url =$_SERVER['REQUEST_URI'];
		$url = strtok($url, '?');
		
		$admin_bar->add_menu( array(
			'id'    => 'my-third-sub-item2',
			'parent' => 'my-item',
			'title' => 'Перевести PL => EN',
			'href'  => "http://".$_SERVER['HTTP_HOST'].$url."?go_translate_pl=1",
			'meta'  => array(
				'title' => __('My Second Sub Menu Item aa'),
				'target' => '_blank',
				'class' => 'my_menu_item_class2'
			),
		));
	}*/
}
...
function pro_shortcode( $atts, $content = null ) {
    $output = '<span class="pro-shortcode">';
    $output .= wp_strip_all_tags( $content );
    $output .= '<button class="pro-shortcode-button" onclick="proCopyToClipboard(this)"><img src="/wp-content/uploads/2023/08/copy.svg" width="16" height="16" title="Скопировать"></button>';
    $output .= '</span>';
    return $output;
}
...
// Если у поста есть ACF-поле, связывающее с казино, то включаем галку поста "Не показывать на русской версии"
add_action( 'save_post', 'er_force_turn_off_on_ru_language' );
function er_force_turn_off_on_ru_language( $post_id ) {

	if ( ! is_admin() ) {
		return;
	}

	$company_type_id = get_field( 'company_type', $post_id );

	if( $company_type_id ) {

		$term = get_term( $company_type_id );
		if( isset( $term->name ) and $term->name == 'onlinecasino' ) {
		
			update_field( 'turn_off_on_ru_language', 'yes', $post_id );
...
/*function remove_wp_ver_css_js( $src ) {
	if (is_admin()) {
		if ( strpos( $src, 'ver=' ) ) {
			//$src = str_replace( 'ver=','ver=1', $src );
			//$src = str_replace(site_url("/wp-includes"), "https://eto-razvod.net/wp-includes", $src);
		}
	}
...
function wpse_display_moderator( $comment_text, $comment ) {
	...
	if ( $approved_by ) {
		$comment_text .= "<span class=\"casfqq\" style=\"background: #646970; color: #FFF;padding: 5px;display: inline-block;\"> - Комментарий одобрен: $approved_by</span>";
	}
	return $comment_text;
}
add_filter( 'comment_text', 'wpse_display_moderator', 99, 2 );

if ( ! function_exists( 'upload_images_for_games' ) ) {
	/**
	 * Upload image to special folder for games (special NO WP media gallery)
	 *
	 * @param int|string $url
	 *
	 * @return int|string
	 */
	function upload_images_for_games( $url ) {
		...
		if ( is_wp_error( $tmp ) ) {
			...
			if ( strpos( basename( $url ), '?' ) !== false ) {
				$name = bin2hex( random_bytes( 5 ) ) . '-' . substr( basename( $url ), 0, strpos( basename( $url ), '?' ) );
			} else {
				$name = bin2hex( random_bytes( 5 ) ) . '-' . basename( $url );
			}
			...
			$chars = str_shuffle( "1234567890" );
			$id    = wp_upload_bits( $file_array['name'], null, file_get_contents( $tmp ) );
...
// Выключение скриптов CF7 на всех страницах, где не вставлены формы CF7
...
// Замена домена в строке на домен для выбранного языка
function replace_ru_domain_for_lang( $url, $locale ) {

	switch( $locale ) {

		case 'en_US':
			$url = str_replace( "eto-razvod.ru", "revieweek.com", $url );
			$url = str_replace( "etorazvod.ru", "revieweek.com", $url );
		break;
		case 'es_ES':
			$url = str_replace( "eto-razvod.ru", "revieweek.es", $url );
			$url = str_replace( "etorazvod.ru", "revieweek.es", $url );
		break;
		case 'de_DE':
			$url = str_replace( "eto-razvod.ru", "revieweek.de", $url );
			$url = str_replace( "etorazvod.ru", "revieweek.de", $url );
		break;
		case 'fr_FR':
			$url = str_replace( "eto-razvod.ru", "revieweek.fr", $url );
			$url = str_replace( "etorazvod.ru", "revieweek.fr", $url );
		break;
		case 'pl_PL':
			$url = str_replace( "eto-razvod.ru", "pl.revieweek.com", $url );
			$url = str_replace( "etorazvod.ru", "pl.revieweek.com", $url );
		break;
		case 'fi':
			$url = str_replace( "eto-razvod.ru", "fi.revieweek.com", $url );
			$url = str_replace( "etorazvod.ru", "fi.revieweek.com", $url );
		break;
		case 'id_ID':
			$url = str_replace( "eto-razvod.ru", "id.revieweek.com", $url );
			$url = str_replace( "etorazvod.ru", "id.revieweek.com", $url );
		break;
...
// Получение по id пользователя и локали альтернативного имени пользователя
// Физический смысл: для показа альт.имени редактора на бурже
function get_user_alt_name( $user_id, $locale ) {

	if($locale == 'ru_RU') {
		return '';
	}

	return get_field('user_alt_name', 'user_'. $user_id );

}

// Изменение ссылки Etorazvod.ru для буржа
function change_main_site_link( $content ) {
	$locale = get_locale();
	switch ( $locale ) {
		case 'en_US':
			$content = str_replace( "etorazvod.ru", "revieweek.com", $content );
			break;
		case 'es_ES':
			$content = str_replace( "etorazvod.ru", "revieweek.es", $content );
			$content = str_replace( "revieweek.com", "revieweek.es", $content );
			break;
		case 'de_DE':
			$content = str_replace( "etorazvod.ru", "revieweek.de", $content );
			$content = str_replace( "revieweek.com", "revieweek.de", $content );
			break;
		case 'fr_FR':
			$content = str_replace( "etorazvod.ru", "revieweek.fr", $content );
			$content = str_replace( "revieweek.com", "revieweek.fr", $content );
			break;
		case 'pl_PL':
			$content = str_replace( "etorazvod.ru", "pl.revieweek.com", $content );
			//$content = str_replace( "revieweek.com", "pl.revieweek.com", $content );
			break;
		case 'fi':
			$content = str_replace( "etorazvod.ru", "fi.revieweek.com", $content );
			//$content = str_replace( "revieweek.com", "fi.revieweek.com", $content );
			break;
		case 'id_ID':
			$content = str_replace( "etorazvod.ru", "id.revieweek.com", $content );
			//$content = str_replace( "revieweek.com", "id.revieweek.com", $content );
			break;
		case 'ru_RU':
			break;
		default:
			$content = str_replace( "etorazvod.ru", "", $content );
...
add_filter( 'the_content', 'change_main_site_link' );
...
function add_alternate_link_tag() {
	...
	printf( "\n<link rel=\"alternate\" hreflang=\"kk-kz\" href=\"https://revieweek.kz/%s\" />\n", $path );
}
add_action('wp_head', 'add_alternate_link_tag');

//as
add_action( 'comment_unapproved_to_approved', 'my_comment_approved_function' );
function my_comment_approved_function( $new_comment ) {
	$message = 'Спасибо за ваш отзыв на нашем сайте. Он был одобрен администратором и теперь виден всем посетителям.';
	$headers = 'From: admin@mysite.com' . "\r\n";
	wp_mail( 'alador@yandex.ru', 'Ваш отзыв одобрен', $message, $headers );
}
