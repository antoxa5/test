...
function auth_action() {
    ...
    if (typeof click_sidebar !== 'undefined') {
        if (click_sidebar == 99999999999) {
            window.location.href = '/dashboard-add-company/';//https://etorazvod.ru/dashboard/services/blog/
        } else {
...
function remove_promo(id, num, company_id, company_slug) {
    $.ajax({
        ...
        success: function (data) {
            result = data;
            popup_alert_message(__('Промокод успешно удалён! Он', 'er_theme'), 'ok');
        }
    });
}
...
function popup_alert_message(message, type) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (type == 'copy_widget') {
                $('#popup_copy_widget textarea').val('<a href="https://etorazvod.ru/"><img src="https://etorazvod.ru/widget/rate_img_mini.png?id=' + $('#popup_search_companies input[name="autocomplete_result"]').val() + '" alt=""></a>');
            }
        }
    });
}
...
function savesendingmails(site, all, tematics) {
    $.ajax({
        ...
        success: function (data) {
            ...
            console.log(data);
            popup_alert_message(__('Настройки уведомлений и рассылок успешно сохранены!', 'er_theme'), 'ok');
        }
    });
...
function ajax_upload_file_2(file_data, append_id) {
    ...
    if (res[0].file.ContentLength > 2097152) {
        ...
        popup_alert_message(__('Количество загруженных файлов не может превышать 10 штук', 'er_theme'), 'error');
    } else {
        $.ajax({
            url: my_ajax_object.ajax_url,
            type: 'POST',
...
function proCopyToClipboard(button) {
    var text = button.previousSibling.textContent;
    navigator.clipboard.writeText(text);
    button.innerHTML = '<img src="/wp-content/uploads/2023/08/checkboxes.svg" width="16" height="16">';
}
...
jQuery(document).ready(function ($) {
    ...
    $('.invest_form select').on('change', function () {
        ...
        if (this.value != option_first_child) {
            ...
            placeholder_main = __('Логин', 'er_theme');
    ...
    if (window.location.href.indexOf("/misson/") > -1) {
        if ($('body').hasClass('logged_in')) {
            ...
            $('.reg_btn_main').text(__('Войти в личный кабинет', 'er_theme'));
        } else {
    ...
    if (typeof meta_page === 'undefined') {
        ...
        if (pathname.indexOf('/user/') != -1) {
            if (typeof window.ym != 'undefined') {
                ym(93311015, 'reachGoal', 'userprofile');
    ...
    $('body').on('click', '.get_cat_add_company .autocomplete_search_results ul li', function () {
        ...
        $('.go_next_add_company').remove();
        $('.bg_add_company_insert').append('<div class="button button_violet font_small font_bold  go_next_add_company pointer">' + __('Далее', 'er_theme') + '</div>')
    });
    ...
    $('body').on('click', '.finish_btn_add_company', function (e) {
        ...
        if ($('select[name="select_contact"]').val() == __('Мессенджер для связи', 'er_theme')) {
            $('.finish_btn_add_company').removeClass('go_next_add_company_act');
            $('.finish_btn_add_company .load_ajax').remove();
            popup_alert_message(__('Вы не выбрали мессенджер для связи', 'er_theme'), 'error');
        } else {
            if ($('input[name="contact_name"]').val() == '') {
                $('.finish_btn_add_company').removeClass('go_next_add_company_act');
                $('.finish_btn_add_company .load_ajax').remove();
                popup_alert_message(__('Вы не заполнили логин', 'er_theme') + ' ' + contact_type, 'error');
            } else {
                        ...
                        if (verify_method == 'pay') {
                            $.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    window.location.href = '/dashboard/wallet/?price=4990&id=' + result.poster_id;
                                }
                            });
                        } else {
    ...
    $('body').on('change', 'input[name="verify_method"]', function () {
        if ($('input[name="verify_method"]:checked').attr('id') == 'pay') {
            $('.finish_btn_add_company').text(__('Далее', 'er_theme'));
        } else {
            popup_alert_message('', 'copy_widget');
            $('.finish_btn_add_company').text(__('Отправить на модерацию', 'er_theme'));
        }
    });
    ...
    if (getUrlParameter('activation') == 'ok') {
        popup_alert_message(__('Поздравляем! Вы подтвердили профиль!', 'er_theme'), 'ok');
    }

    if (getUrlParameter('activation') == 'ok_email') {
        popup_alert_message(__('Новый E-mail адрес успешно привязан', 'er_theme'), 'ok');
    }

    if (getUrlParameter('activation') == 'error') {
        popup_alert_message(__('Произошла ошибка активации, обратитесь за помощью на e-mail адрес - info@eto-razvod.ru', 'er_theme'), 'error');
    }

    if (getUrlParameter('activation') == 'error_email') {
        popup_alert_message(__('Произошла ошибка подтверждения нового E-mail адреса, обратитесь за помощью на e-mail адрес - info@eto-razvod.ru', 'er_theme'), 'error');
    }

    if (getUrlParameter('company')) {
        $('.bg_add_company input').attr('autocomplete', 'off');
        $('.bg_add_company input').attr('autocorrect', 'off');
        $('.bg_add_company input').attr('autocapitalize', 'off');
        $('.bg_add_company input').attr('spellcheck', 'false');
    }
    ...
    $('body').on('input', 'input[name="autocomplete_text"]', function () {
        ...
        if (search_type === 'filter_ratings') {
            ...
            var tags_values = $('#' + block_id + ' .outside_tags input[name="tags[]"]').map(function () {
                return $(this).val();
            }).get();
    ...
    $('body').on('click', '.icon_bookmark', function () {
        ...
        if ($(this).hasClass('active')) {
            ...
            if (window.sidebar && window.sidebar.addPanel) { // Mozilla Firefox Bookmark
                ...
                popup_alert_message(sprintf(__('Нажмите %s и D для добавления страницы в Закладки.', 'er_theme'), (navigator.userAgent.toLowerCase().indexOf('mac') != -1 ? 'Command/Cmd' : 'CTRL')), 'ok');
            }
        }
    ...
    $('body').on('click', 'div.comment_avatar, div.comment-author', function () {
        var link = $(this).attr('data-link');
        console.log($(this).attr('data-link') + '444');
        if ((link == 'https://etorazvod.ru/user/') || (link == 'https://etorazvod.ru/user//')) {
        ...
        if (my_ajax_object.current_post_id != "150534") {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#sa_new_form_for_add_company input[name="add_company_name"]').val(name);
                }
            });
            /*name = $('div[data-form-type="reviewgetcompany"] input[name="autocomplete_text"]').val();
             $.ajax({
    ...
    $('body').on('click', '.link_promocode_text_copy', function () {
        ...
        // }
        $(this).text(__('Скопировано', 'er_theme'));
    ...
    $('body').on('click', '.copy_code_widget', function () {
        ...
        // }
        $('.copy_code_widget').text(__('Скопировано', 'er_theme'));
    ...
    $('body').on('click', 'a[href*="visit"]', function () {
        ...
        yaCounter93311015.reachGoal('VISIT', {URL: document.location.href, offer: offer_slug});
        if ($(this).parent().parent().attr('class') == 'fixed_header_review fixed_header_review_2 fixed_header_review_3') {
            yaCounter93311015.reachGoal('VISITFIXEDHEADER', {URL: document.location.href, offer: offer_slug});
        } else if ($(this).hasClass('review_block_main_button')) {
            yaCounter93311015.reachGoal('VISITBEFOREFIXED', {URL: document.location.href, offer: offer_slug});
        }
    });
    ...
    $('body').on('click', '.review_card a[href*="visit"]', function () {
        ...
        var offer_slug = this.href.split("/")[4];
        yaCounter93311015.reachGoal('CARD', {URL: document.location.href, offer: offer_slug});
    ...
    $('body').on('click', '.top_companies_heygo_best_com a[href*="visit"]', function () {
        ...
        var offer_slug = this.href.split("/")[4];
        yaCounter93311015.reachGoal('POPUPTOP', {URL: document.location.href, offer: offer_slug});
    ...
    $('body').on('click', '.top_companies_heygo_gift a[href*="visit"]', function () {
        ...
        var offer_slug = this.href.split("/")[4];
        yaCounter93311015.reachGoal('GIFTFORREVIEW', {URL: document.location.href, offer: offer_slug});
    ...
    $('body').on('click', '.popup_link_main .popup_main_link_main', function () {
        if ($(this).hasClass('popup_quiz')) {
            ...
            console.log('POPUP')
            var offer_slug = this.href.split("/")[4];
            yaCounter93311015.reachGoal('POPUP', {URL: document.location.href, offer: offer_slug});
    ...
    $('body').on('click', '.social_links_quiz > .link.telegram', function () {
        ym(93311015, 'reachGoal', 'TELEGRAM');
        let id = $(this).attr('data-id');
        if (id != "") {
            $.ajax({
                url: my_ajax_object.ajax_url,
    ...
    $('body').on('click', '.popup_window ul.heygo_promocodes .get_promocode_1', function () {
        ...
        var offer_slug = this.href.split("/")[4];
        yaCounter93311015.reachGoal('POPUP_PROMOCODE', {URL: document.location.href, offer: offer_slug});
    ...
    $('body').on('click', '#hey_content_review_reviews_1 a[href*="visit"], #hey_content_review_reviews_2 a[href*="visit"]', function () {
        ...
        var offer_slug = this.href.split("/")[4];
        yaCounter93311015.reachGoal('NATIVECOMMENTS', {URL: document.location.href, offer: offer_slug});
    ...
    $('body').on('click', '#hey_content_review_content_1 a[href*="visit"], #hey_content_review_content_2 a[href*="visit"]', function () {
        ...
        var offer_slug = this.href.split("/")[4];
        yaCounter93311015.reachGoal('NATIVEREVIEW', {URL: document.location.href, offer: offer_slug});
    ...
    $('body').on('click', '#hey_content_rating_1 a[href*="visit"], #hey_content_rating_2 a[href*="visit"]', function () {
        ...
        var offer_slug = this.href.split("/")[4];
        yaCounter93311015.reachGoal('NATIVERATING', {URL: document.location.href, offer: offer_slug});
    ...
    $('body').on('click', '.row_buttons a[href*="visit"]', function () {
        ...
        var offer_slug = this.href.split("/")[4];
        yaCounter93311015.reachGoal('RATINGBUTTON', {URL: document.location.href, offer: offer_slug});
    ...
    $('body').on('click', '.similar_container a[href*="review"]', function () {
        ...
        var offer_slug = this.href.split("/")[4];
        yaCounter93311015.reachGoal('SIMILARMAIN', {URL: document.location.href, offer: offer_slug});
    ...
    $('body').on('click', '.side_block .company_in_rating_item a[href*="review"]', function () {
        ...
        var offer_slug = this.href.split("/")[4];
        yaCounter93311015.reachGoal('SIMILARSIDE', {URL: document.location.href, offer: offer_slug});
    ...
    $('body').on('click', '.container_left .promocode_list_single_right .promocode_button_container a', function () {
        ...
        var offer_slug = this.href.split("/")[4];
        yaCounter93311015.reachGoal('PROMOCODESINGLE', {URL: document.location.href, offer: offer_slug});
    ...
    $('body').on('click', '.block_reviews_list .promocode_button_container a, .er_list_promocodes .promocode_button_container a, .list_promocodes .promocode_button_container a', function () {
        ...
        var offer_slug = this.href.split("/")[4];
        yaCounter93311015.reachGoal('PROMOCODELIST', {URL: document.location.href, offer: offer_slug});
    ...
    $('body').on('click', '#reviewsummary a[href*="visit"]', function () {
        console.log('TABLE');
        var offer_slug = this.href.split("/")[4];
        yaCounter93311015.reachGoal('TABLE', {URL: document.location.href, offer: offer_slug});
    ...
    $('body').on('click', '.review_container_content a[href*="visit"]', function () {
        console.log('CONTENT');
        var offer_slug = this.href.split("/")[4];
        yaCounter93311015.reachGoal('CONTENT', {URL: document.location.href, offer: offer_slug});
    ...
    $('body').on('click', '.review_sidebar_banner a[href*="visit"]', function () {
        console.log('SIDEBAR');
        var offer_slug = this.href.split("/")[4];
        yaCounter93311015.reachGoal('SIDEBAR', {URL: document.location.href, offer: offer_slug});
        ...
        $.post(
                ...
                function (data) {
                    ...
                    if (result.status === 'error') {
                        ...
                        text1 = __('Войти', 'er_theme');

                        $('#after_login_new_comment_form input[type="submit"]').val(__('Войти', 'er_theme'));
                        ...
                        // }
                        $('#after_login_new_comment_form input[type="submit"]').val(__('Завершить регистрацию', 'er_theme'));
    ...
    $('body').on('click', '.button_review_link_comment', function () {
        ym(93311015, 'reachGoal', 'CLICK_OFFER_FROM_COMMENT');
    });
    ...
    $(window).scroll(function () {
        ...
        if ($('.active_fixed_page__single_review_2').length != 0) {
            ...
            if ($('.review_sidebar_banner.sticky').length != 0) {
                if ($(window).width() > 1210) {
                    if ($('body').hasClass('logged_in')) {
                        $('.review_sidebar_banner.sticky').css('top', '145px');
                    } else {
                        $('.review_sidebar_banner.sticky').css('top', '105px');
                    }
                }
    ...
    // 07.09.23
    $('body').on('submit', '.active_form#sa_new_form_for_add_company', function (e) {
        e.preventDefault();
    });

    $('body').on('submit', '.inactive_form#sa_new_form_for_add_company', function (e) {
        e.preventDefault();
        $('#sa_new_form_for_add_company .status_message').remove();
        $('#sa_new_form_for_add_company').append('<div class="spinner spinner_blue"></div>');
        $('#sa_new_form_for_add_company .spinner').show();
        var form = $(this);
        $(this).removeClass('inactive_form');
        $(this).addClass('active_form');
        $.post(form.attr('action'), form.serialize(), function (data) {
            $('#sa_new_form_for_add_company .spinner').remove();
            result = JSON.parse(data);
            if (result.status === 'ok') {
                //alert(result.message);
                //$('#comment_form textarea').val('');
                $('#sa_new_form_for_add_company').append('<span class="status_message font_smaller m_t_20 m_b_20 color_green reg_error">' + result.message + '</span>');
                $('#sa_new_form_for_add_company').removeClass('active_form');
                $('#sa_new_form_for_add_company').addClass('inactive_form');
                $('#sa_new_form_for_add_company input[name="add_company_name"]').val('');
                $('#sa_new_form_for_add_company input[name="contact_name"]').val('');
            } else if (result.status === 'error') {

                $('#sa_new_form_for_add_company').append('<span class="status_message font_smaller m_b_20 color_red reg_error">' + result.message + '</span>');
                $('#sa_new_form_for_add_company').removeClass('active_form');
                $('#sa_new_form_for_add_company').addClass('inactive_form');
            }
        });
    });
    //$('#popup_form_review_newform .rating input[type="radio"]').on("click", function () {
    $('body').on('click', '#popup_form_review_newform .rating input[type="radio"]', function () {
        ...
        if (ratings.length !== counter.length) {
            return;
        } else {
            if (counter.includes("1") && ratings.length === counter.length) {
                var negative_title = $('#popup_form_review_newform #ratestatus .notice-title').attr('attr-negative');
                var negative_text = $('#popup_form_review_newform #ratestatus .color_dark_blue').attr('attr-negative');
                $('#popup_form_review_newform #ratestatus .notice-title').empty();
                $('#popup_form_review_newform #ratestatus .notice-title').text(negative_title);
                $('#popup_form_review_newform #ratestatus .color_dark_blue').empty();
                $('#popup_form_review_newform #ratestatus .color_dark_blue').html(negative_text);
                $("#ratestatus").show();
            } else {
                $("#ratestatus").hide();
            }
    ...
    $('#power_widget_small').click(function () {
        var comp_id = $('#hidden_comp_id').val();
        $.ajax({
            url: my_ajax_object.ajax_url,
            type: "POST",
            data: "action=init_widget_small&comp_id="+comp_id,
            dataType: 'html',
            beforeSend: function (xhr) {
...
function ajax_get_widget_small() {

    console.log(my_ajax_object.ajax_url);
    $.ajax({
        url: my_ajax_object.ajax_url,
        type: "POST",
        data: "action=get_reting_casino",
        dataType: 'html',
        beforeSend: function (xhr) {
