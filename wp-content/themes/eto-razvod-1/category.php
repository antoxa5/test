...
if( $cid ) {
	query_posts( $query_string . '&meta_key=news_for_company_id&meta_value=' . $cid );
}
...
if( $current_language == 'ru_RU' && get_field('tag_hide_on_ru_language', $category) == 'yes' ) {
    $wp_query->set_404();
    status_header( 404 );
    nocache_headers();
    include("404.php");
...
$nowlink = "https://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
$permalink = get_category_link($category->term_id);

$nowlink__temp = stristr($nowlink, '?', true);
if ($nowlink__temp == '') {
	$nowlink__temp = $permalink;
}

$nowlink__temp_2 = stristr($nowlink, '/page', true);
if ($nowlink__temp_2 == '') {
	$nowlink__temp_2 = $permalink;
} else {
	$nowlink__temp_2 = $nowlink__temp_2.'/';
}

$nowlink__temp_3 = stristr($nowlink, '?', true);
if ($nowlink__temp_3 == '') {
	$nowlink__temp_3 = $nowlink;
}

if( is_paged() ){
	if ($nowlink__temp_2 == $permalink) {
	...
	if ( ($nowlink__temp_3 == $permalink) && ($nowlink__temp == $permalink)) {
...
if (have_posts()) :
    ...
    <div class="page_header news_page_top news_page_top_5">
		...
		if (function_exists('show_breadcrumbs')) {
			$news_header .= show_breadcrumbs();
		}
		$news_header .= '<div class="page_heading_line">';
		$news_header .= '<h1 class="color_dark_blue flex font_medium_new">'.single_cat_title('', 0).'</h1>';
            ...
            <div class="wrap">
                    <span class="link_news_filter icon_bg icon_filter pointer border_circle flex m_r_50 color_dark_gray font_smaller"><?php _e('Фильтр','er_theme'); ?></span>
                     <ul class="news_tab_selector">
                    	<li><a href="<?php bloginfo('url');?>/pages/"><?php _e('Статьи','er_theme'); ?></a></li>
                    	<li class="active"><span><?php _e('Блог','er_theme'); ?></span></li>
                    </ul>
